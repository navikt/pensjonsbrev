...
object Letter2Markup : LetterRenderer<LetterWithAttachmentsMarkup>() {
    ...
    fun renderLetterOnly(
        ...
        LetterMarkup(
            ...
            signatur =
                scope.felles.signerendeSaksbehandlere.let { sign ->
                    Signatur(
    ...
    private fun renderOutlineContent(
        scope: ExpressionScope<*>,
    ...
    private fun renderForm(
        scope: ExpressionScope<*>,
        form: Element.OutlineContent.ParagraphContent.Form<*>,
    ): ParagraphContent.Form =
        when (form) {
            ...
            is Element.OutlineContent.ParagraphContent.Form.MultipleChoice ->
                ParagraphContent.Form.MultipleChoice(
                    id = form.stableHashCode(),
                    prompt = renderText(scope, listOf(form.prompt)),
    ...
    private fun renderHeader(
        scope: ExpressionScope<*>,
        header: Element.OutlineContent.ParagraphContent.Table.Header<*>,
    ): ParagraphContent.Table.Header =
        ParagraphContent.Table.Header(
            header.stableHashCode(),
            header.colSpec.map { columnSpec ->
                ParagraphContent.Table.ColumnSpec(
                    id = columnSpec.stableHashCode(),
                    headerContent = renderCell(scope, columnSpec.headerContent),
                    alignment =
                        when (columnSpec.alignment) {
    ...
    ): List<ParagraphContent.Table.Row> =
        buildList {
    ...
    private fun renderCell(
        scope: ExpressionScope<*>,
        cell: Element.OutlineContent.ParagraphContent.Table.Cell<*>,
    ): ParagraphContent.Table.Cell =
        ParagraphContent.Table.Cell(cell.stableHashCode(), renderText(scope, cell.text))

    private fun renderItemList(
        scope: ExpressionScope<*>,
        itemList: Element.OutlineContent.ParagraphContent.ItemList<*>,
    ): ParagraphContent.ItemList? =
        buildList {
    ...
    private fun StringExpression.toContent(
        scope: ExpressionScope<*>,
        fontType: FontType,
    ): List<Text> =
    ...
    private fun canMergeAsLiterals(
        first: Text?,
        second: Text,
    ): Boolean {
