package no.nav.brev.brevbaker.markup.dsl.extended

import no.nav.brev.brevbaker.markup.dsl.AbstractContentBuilder
import no.nav.brev.brevbaker.markup.dsl.MarkupDsl
import no.nav.brev.brevbaker.markup.MarkupModel
import no.nav.brev.brevbaker.markup.outline.EditBehaviour
import no.nav.brev.brevbaker.markup.outline.Text
import no.nav.brev.brevbaker.markup.outline.Text.FontType

/**
 * Tekstinnhold som også tillater [variable] og tags. Hvert element krever en eksplisitt [Int]-id –
 * denne modulen genererer aldri id-er.
 */
@ExtendedMarkupDsl
@MarkupDsl
class ExtendedContentBuilder internal constructor() : AbstractContentBuilder() {
    /**
     * Legg til fast tekst, valgfritt med [fontType] og [editBehaviour].
     *
     * ```
     * text(1, "Du får ")
     *
     * text(2, "fritekst", editBehaviour = EditBehaviour.FRITEKST)
     * ```
     */
    fun text(id: Int, text: String, fontType: FontType = FontType.PLAIN, editBehaviour: EditBehaviour? = null) {
        texts.add(MarkupModel.literal(id, text, fontType, editBehaviour))
    }

    /**
     * Legg til en variabel (datafelt som fylles ut ved rendring), valgfritt med [fontType] og [editBehaviour].
     *
     * ```
     * variable(2, "1000 kr", editBehaviour = EditBehaviour.REDIGERBAR_DATA)
     * ```
     */
    fun variable(id: Int, text: String, fontType: FontType = FontType.PLAIN, editBehaviour: EditBehaviour? = null) {
        texts.add(MarkupModel.variable(id, text, fontType, editBehaviour))
    }

    /**
     * Legg til et linjeskift.
     *
     * ```
     * newLine(3)
     * ```
     */
    fun newLine(id: Int) {
        texts.add(MarkupModel.newLine(id))
    }
}

/**
 * Begrenset tekst-scope for overskrifter/ledetekster (titler, kolonneoverskrifter, form-choice-ledetekst).
 * Tillater kun ren tekst: [text] og [variable] (alltid [FontType.PLAIN], ingen linjeskift). Hvert element
 * krever en eksplisitt [Int]-id.
 */
@ExtendedMarkupDsl
@MarkupDsl
class PlainExtendedTextBuilder internal constructor() {
    internal val texts: MutableList<Text> = mutableListOf()

    /**
     * Vanlig tekst.
     *
     * ```
     * text(1, "Vedtak")
     * ```
     */
    fun text(id: Int, text: String) {
        texts.add(MarkupModel.literal(id, text, FontType.PLAIN))
    }

    /**
     * Legg til en variabel.
     *
     * ```
     * variable(2, "navn")
     * ```
     */
    fun variable(id: Int, text: String) {
        texts.add(MarkupModel.variable(id, text, FontType.PLAIN))
    }

    internal fun build(): List<Text> = texts.toList()
}

/** Ren tekst med `variable` (kun [Text.Literal]/[Text.Variable], [FontType.PLAIN], ingen linjeskift). */
internal fun plainExtendedText(build: PlainExtendedTextBuilder.() -> Unit): List<Text> =
    PlainExtendedTextBuilder().apply(build).build()
