...
): Saksinformasjon = MarkupModel.saksinformasjon(
...
): Signatur = MarkupModel.signatur(
    saksbehandlerSignatur = saksbehandlerNavn?.let {
        MarkupModel.saksbehandlerSignatur(it, attesterendeSaksbehandlerNavn)
    },
...
fun pdfTittel(content: ContentBuilder.() -> Unit): PDFTittel =
    MarkupModel.pdfTittel(ContentBuilder().apply(content).build())
...
class LetterMarkupBuilder<C : AbstractContentBuilder> internal constructor(
    ...
    internal fun build(): LetterMarkup = MarkupModel.letterMarkup(
        title1 = title1,
        saksinformasjon = saksinformasjon,
        blocks = blocks,
        signatur = signatur,
...
class AttachmentBuilder<C : AbstractContentBuilder> internal constructor(
    ...
    internal fun build(): Attachment = MarkupModel.attachment(title1, blocks, inkluderSaksinformasjon)
}
