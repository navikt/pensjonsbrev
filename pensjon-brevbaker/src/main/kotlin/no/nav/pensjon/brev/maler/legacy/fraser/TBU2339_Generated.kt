...
data class TBU2339_Generated(
    ...
    override fun OutlineOnlyScope<LangBokmalNynorskEnglish, Unit>.template() {
		...
		paragraph {
			textExpr (
				Bokmal to "Inntekten til deg og ".expr() + pe.sivilstand_ektefelle_partner_samboer_bormed_ut_alle_spraak_entall() + " din har betydning for hva du får i barnetillegg. Er inntektene over grensen for å få utbetalt fullt barnetillegg, blir tillegget redusert. ",
			...
			showIf((pe.vedtaksdata_beregningsdata_beregning_beregningytelsekomp_barnetilleggserkull_btsbinnvilget() and pe.vedtaksdata_beregningsdata_beregning_beregningytelsekomp_barnetilleggfelles_btfbinnvilget())){
				textExpr (
					Bokmal to "Inntekten til  ".expr() + pe.sivilstand_ektefelle_partner_samboer_bormed_ut_alle_spraak_entall() + " din har kun betydning for størrelsen på barnetillegget til " + pe.ut_barnet_barna_felles() + " som bor sammen med begge sine foreldre.",
