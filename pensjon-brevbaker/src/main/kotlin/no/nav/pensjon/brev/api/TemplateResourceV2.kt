...
class TemplateResourceV2<Kode : Enum<Kode>, out T : BrevTemplate<BrevbakerBrevdata, Kode>>(
    val name: String,
    templates: Set<T>,
    ...
    suspend fun renderPDF(call: ApplicationCall, brevbestilling: BestillBrevRequest<Kode>): LetterResponse.V2 =
        with(brevbestilling) {
            renderPDF(call, createLetter(kode, letterData, language, felles))
        }

    suspend fun renderPDF(call: ApplicationCall, brevbestilling: BestillRedigertBrevRequest<Kode>): LetterResponse.V2 =
        with(brevbestilling) {
            renderPDF(call, createLetter(kode, letterData, language, felles), letterMarkup)
        }

    fun renderHTML(brevbestilling: BestillBrevRequest<Kode>): LetterResponse.V2 =
        with(brevbestilling) {
            renderHTML(createLetter(kode, letterData, language, felles))
        }

    fun renderHTML(brevbestilling: BestillRedigertBrevRequest<Kode>): LetterResponse.V2 =
        with(brevbestilling) {
    ...
    private suspend fun renderPDF(call: ApplicationCall, letter: Letter<BrevbakerBrevdata>, redigertBrev: LetterMarkup? = null): LetterResponse.V2 =
    ...
    private fun renderHTML(letter: Letter<BrevbakerBrevdata>, redigertBrev: LetterMarkup? = null): LetterResponse.V2 =
        renderCompleteMarkup(letter, redigertBrev)
