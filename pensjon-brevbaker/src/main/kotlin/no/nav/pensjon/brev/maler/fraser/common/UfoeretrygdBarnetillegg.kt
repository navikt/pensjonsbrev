...
object UfoeretrygdBarnetillegg {
    ...
    // TBU2339
    data class BetydningAvInntektFellesbarnSaerkullsbarn(
        val harBarnetilleggFellesbarn: Expression<Boolean>,
        val harBarnetilleggSaerkullsbarn: Expression<Boolean>,
        val sivilstand: Expression<Sivilstand>
    ...
    // TBU1286
    data class HarSaerkullsbarnOgHarFellesbarnOgBarnetilleggForSaerkullbarnBlirRedusertEllerIkke(
        val beloepNettoSaerkullsbarn: Expression<Kroner>,
        ...
        override fun OutlineOnlyScope<LangBokmalNynorskEnglish, Unit>.template() {
            paragraph {
                ...
                // Bruker mottar barnetillegg for både saerkullsbarn og fellesbarn
                showIf(harBarnetilleggSaerkullsbarn and harBarnetilleggSaerkullsbarn) {
                    // Kun barnetillegget for saerkullsbarn er blitt redusert eller ikke redusert
                    showIf(
                        beloepNettoSaerkullsbarn.notEqualTo(0) and justeringsbeloepSaerkullsbarn.equalTo(0))
                    {
                        val hoeyereLavere = inntektBruktIAvkortningSaerkullsbarn.greaterThan(fribeloepSaerkullsbarn)
                        val redusert = fradragSaerkullsbarn.greaterThan(0)
                        textExpr(
                            Bokmal to "Inntekten din er ".expr() + ifElse(
                                hoeyereLavere,
                                ifTrue = "høyere",
                                ifFalse = "lavere"
                            ) + " enn ".expr() + fribeloep + " kroner, som er fribeløpet for barnetillegget til ".expr() + ifElse(
                                barnFlertall,
                                ifTrue = "barna",
                                ifFalse = "barnet"
                            ) + " som ikke bor sammen med begge foreldrene. Dette barnetillegget er derfor ".expr() + ifElse(
                                redusert,
                                ifTrue = "redusert",
                                ifFalse = "ikke redusert"
                            ) + " ut fra inntekt.".expr(),
                            Nynorsk to "Inntekta di er ".expr() + ifElse(
                                hoeyereLavere,
                                ifTrue = "høgare",
                                ifFalse = "lågare"
                            ) + " enn ".expr() + fribeloep + " kroner, som er fribeløpet for barnetillegget til ".expr() + ifElse(
                                ...
                                ifTrue = "barna",
                                ifFalse = "barnet"
                            ) + " som ikkje bur saman med begge foreldra. Dette barnetillegget er derfor ".expr() + ifElse(
                                redusert,
                                ifTrue = "redusert",
                                ifFalse = "ikkje redusert"
                            ) + " ut frå inntekt.".expr(),
                            English to "Your income is ".expr() + ifElse(
                                hoeyereLavere,
                                ifTrue = "higher",
                                ifFalse = "lower"
                            ) + " higher than NOK ".expr() + fribeloep + ", which is the exemption amount for the child supplement for the ".expr() + ifElse(
                                barnFlertall,
                                ifTrue = "children who do",
                                ifFalse = "child who does"
                            ) + " not live together with both parents. Therefore, your child supplement has ".expr() + ifElse(
                                redusert,
                                ifTrue = "been reduced",
                                ifFalse = "not been reduced"
                            ) + " on the basis of your income.".expr()
                        )
                        showIf(
                            beloepNettoFellesbarn.greaterThan(0) and justeringsbeloepFellesbarn.equalTo(0)
                        ) {
                            textExpr(
                                Bokmal to "Til sammen er inntektene til deg og ".expr(),
                                Nynorsk to "Til saman er inntektene til deg og ".expr(),
                            ...
                            showIf(sivilstand.isOneOf(Sivilstand.GIFT, Sivilstand.GIFT_LEVER_ADSKILT)) {
                                ...
                                text(
                                    Bokmal to "samboeren",
                                    Nynorsk to "sambuaren",
                                    English to "cohabitant partner's"
                                )
                            }
                            val fribeloep = fribeloepFellesbarn.format()
                            val barnFlertall = antallFellesbarnInnvilget.greaterThan(1)
                            val hoeyereLavere = fradragFellesbarn.greaterThan(0)
                            val redusert = fradragFellesbarn.greaterThan(0)
                            textExpr(
                                Bokmal to " din ".expr() + ifElse(
                                    ...
                                    ifTrue = "barna",
                                    ifFalse = "barnet"
                                ) + " som bor med begge sine foreldre. Dette barnetillegget er derfor ".expr() + ifElse(
                                    redusert,
                                    ifTrue = "redusert",
                                    ifFalse = "ikke redusert"
                                ) + " ut fra inntekt.".expr(),
                                Nynorsk to " din ".expr() + ifElse(
                                    ...
                                    ifTrue = "barna",
                                    ifFalse = "barnet"
                                ) + " som bur saman med begge foreldra sine. Dette barnetillegget er derfor ".expr() + ifElse(
                                    redusert,
                                    ifTrue = "redusert",
                                    ifFalse = "ikkje redusert"
                                ) + " ut frå inntekt.".expr(),
                                English to " income is ".expr() + ifElse(
                                    hoeyereLavere,
                                    ifTrue = "higher",
                                    ifFalse = "lower"
                                ) + " than NOK ".expr() + fribeloep + ", which is the exemption amount for the child supplement for the ".expr() + ifElse(
                                    barnFlertall,
                                    ifTrue = "children who live",
                                    ifFalse = "child who lives"
                                ) + " together with both parents. Therefore, your child supplement has been ".expr() + ifElse(
                                    redusert,
                                    ifTrue = "reduced",
                                    ifFalse = "not reduced"
                                ) + " on the basis of your income.".expr()
                        ...
                        textExpr(
                            Bokmal to " din lavere enn ".expr() + fribeloep + " kroner, som er fribeløpet for barnetillegget til ".expr() + ifElse(
                                barnFlertall,
                                ifTrue = "barna",
                                ifFalse = "barnet"
                            ) + " som bor med begge sine foreldre. Dette barnetillegget er derfor ikke redusert ut fra inntekt.".expr(),
                            Nynorsk to " din lågare enn ".expr() + fribeloep + " kroner, som er fribeløpet for barnetillegget til ".expr() + ifElse(
                                barnFlertall,
                                ifTrue = "barna",
                                ifFalse = "barnet"
                            ) + " som bur saman med begge foreldra sine. Dette barnetillegget er derfor ikke redusert ut frå inntekt.".expr(),
                            English to " income is lower than NOK ".expr() + fribeloep + ", which is the exemption amount for the child supplement for the ".expr() + ifElse(
                                barnFlertall,
                                ifTrue = "children who live",
                                ifFalse = "child who lives"
                    ...
                    showIf(
                        ...
                        textExpr(
                            Bokmal to "Inntekten din er høyere enn ".expr() + fribeloepSaerkullsbarn + " kroner, som er fribeløpet for barnetillegget til ".expr() + ifElse(
                                ...
                                ifTrue = "barna",
                                ifFalse = "barnet"
                            ) + " som ikke bor sammen med begge foreldrene. Til sammen er også inntektene til deg og ".expr(),
                            Nynorsk to "Inntekta di er høgare enn ".expr() + fribeloepSaerkullsbarn + " kroner, som er fribeløpet for barnetillegget til ".expr() + ifElse(
                                ...
                                ifTrue = "barna",
                                ifFalse = "barnet"
                            ) + " som ikkje bur saman med begge foreldra. Til saman er også inntektene til deg og ".expr(),
                            English to "Your income is higher than NOK ".expr() + fribeloepSaerkullsbarn + ", which is the exemption amount for the child supplement for the ".expr() + ifElse(
                                ...
                                ifTrue = "children who do",
                                ifFalse = "child who does"
                            ) + " not live together with both parents. Together, your income and your ".expr()
                        )
                        showIf(sivilstand.isOneOf(Sivilstand.GIFT, Sivilstand.GIFT_LEVER_ADSKILT)) {
                            text(
                                Bokmal to "ektefellen",
                        ...
                        textExpr(
                            ...
                            English to "Your income is lower than NOK ".expr() + fribeloepSaerkullsbarn + ", which is the exemption amount for the child supplement for the ".expr() + ifElse(
                                ...
                                ifTrue = "children who do",
                                ifFalse = "child who does"
                            ) + " who do not live together with both parents. Together, your income and your ".expr()
                        )
                        showIf(sivilstand.isOneOf(Sivilstand.GIFT, Sivilstand.GIFT_LEVER_ADSKILT)) {
                            text(
                                Bokmal to "ektefellen",
