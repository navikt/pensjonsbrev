...
object LegacyFunksjonsfraser {
    data class PE_UT_fradrag_hoyere_lavere (val pe: Expression<PE>) : ParagraphPhrase<LangBokmalNynorskEnglish>() {
        /*    IF(PE_Vedtaksdata_BeregningsData_BeregningUfore_BeregningYtelsesKomp_BarnetilleggFelles_BTFBfradrag > 0)
        THEN IF UCase(PE_XML_brev_spraak) ='NN' THEN value = "høgare"
        ELSEIF UCase(PE_XML_brev_spraak) ='EN'  THEN value = "higher"
        ELSE  value = "høyere"
        ENDIF

        ELSEIF (PE_Vedtaksdata_BeregningsData_BeregningUfore_BeregningYtelsesKomp_BarnetilleggFelles_BTFBfradrag = 0)
        THEN IF UCase(PE_XML_brev_spraak) ='NN' THEN value = "lågare"
        ...
        override fun ParagraphOnlyScope<LangBokmalNynorskEnglish, Unit>.template() {
            showIf(pe.vedtaksdata_beregningsdata_beregningufore_beregningytelseskomp_barnetilleggfelles_btfbfradrag().greaterThan(0)) {
                text(
                    Bokmal to "høyere",
                    Nynorsk to "høgare",
                    English to "higher"
                )
            }.orShowIf(pe.vedtaksdata_beregningsdata_beregningufore_beregningytelseskomp_barnetilleggfelles_btfbfradrag().equalTo(0)) {
                text(
                    Bokmal to "lavere",
                    Nynorsk to "lågare",
                    English to "lower"
...
ELSEIF (PE_Vedtaksdata_BeregningsData_Beregning_BeregningYtelseKomp_BarnetilleggSerkull_BTSBInntektBruktiAvkortning <= PE_Vedtaksdata_BeregningsData_Beregning_BeregningYtelseKomp_BarnetilleggSerkull_BTSBfribelop)
THEN IF UCase(PE_XML_brev_spraak) ='NN' THEN value = "lågare"
    ELSEIF UCase(PE_XML_brev_spraak) ='EN' THEN value = "lower"
    ELSE  value = "lavere"
    ENDIF
