...
import vedtaksbrev_grunnlag_persongrunnlagsliste_uforetrygdetteroppgjor_uforetrygdetteroppgjordetaljeps_sumfratrekkbt
...
val opplysningerOmETteroppgjoeretUTLegacy = createAttachment<LangBokmalNynorsk, PE>(
    ...
    showIf((pe.vedtaksbrev_vedtaksdata_etteroppgjorresultat_etteroppgjorresultattype().equalTo("tilbakekr") and (pe.pebrevkode().equalTo("PE_UT_23_001") or pe.pebrevkode().equalTo("PE_UT_04_402")))){
        ...
        paragraph {
            textExpr(
                Bokmal to "Hadde du ei slik inntekt i ".expr() + pe.ut_uforetrygdetteroppgjor_periodefom_year().format() + ", må du dokumentere dette innan tre veker frå " + felles.dokumentDato.format() + ". Du treng ikkje sende inn ny dokumentasjon om vi allereie har trekt dette frå. Inntekter som er trekte frå, ser du i tabellen ovanfor.",
                Nynorsk to "Hadde du ei slik inntekt i ".expr() + pe.ut_uforetrygdetteroppgjor_periodefom_year().format() + ", må du dokumentere dette innan tre veker frå " + felles.dokumentDato.format() + ". Du treng ikkje sende inn ny dokumentasjon om vi allereie har trekt dette frå. Inntekter som er trekte frå, ser du i tabellen ovanfor.",
            )
        }
    }
