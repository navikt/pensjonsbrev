...
val vedleggOpplysningerOmEtteroppgjoeret = createAttachment<LangBokmal, OpplysningerOmEtteroppgjoeretDto>(
    ...
    ifNotNull(barnetillegg) {
        ...
        ifNotNull(it.felles) { fellesbarn ->
            title2 {
                text(
        ...
        ifNotNull(it.saerkull) { saerkull ->
            title2 {
                text(
        ...
        paragraph {
            val skulleFaatt =
                it.saerkull.resultat_safe.skulleFaatt_safe.ifNull(Kroner(0)) + it.felles.resultat_safe.skulleFaatt_safe.ifNull(
                    Kroner(0)
                )
            textExpr(
        ...
        paragraph {
            val fikkUtbetalt =
                it.saerkull.resultat_safe.fikk_safe.ifNull(Kroner(0)) + it.felles.resultat_safe.fikk_safe.ifNull(
                    Kroner(0)
                )
            val totaltAvvik =
                it.saerkull.resultat_safe.avvik_safe.ifNull(Kroner(0)) + it.felles.resultat_safe.avvik_safe.ifNull(
                    Kroner(0)
                )
        ...
        val harFellesTillegg = it.felles.notNull()
        ...
        paragraph {
            showIf(it.personinntekt.fratrekk.fratrekk.isNotEmpty()) {
                includePhrase(
                    FratrekkTabell(
                        it.personinntekt.fratrekk,
                        newText(
...
data class FratrekkTabell(val fratrekk: Expression<InntektOgFratrekk.Fratrekk>, val sumText: TextElement<LangBokmal>) :
...
data class InntektTabell(val inntekt: Expression<InntektOgFratrekk.Inntekt>, val sumText: TextElement<LangBokmal>) :
    ParagraphPhrase<LangBokmal>() {
