...
object UfoerOmregningEnslig : StaticTemplate {
    override val template = createTemplate(
        ...
        val avdod_navn = argument().select(UfoerOmregningEnsligDto::avdod_navn)
        val krav_virkedato_fom = argument().select(UfoerOmregningEnsligDto::krav_virkedato_fom)
        val total_ufoeremaaneder = argument().select(UfoerOmregningEnsligDto::total_ufoeremaaneder)
        val minsteytelse_sats_vedvirk = argument().select(UfoerOmregningEnsligDto::minsteytelse_sats_vedvirk)
        val kompensasjonsgrad_ufoeretrygd_vedvirk =
            argument().select(UfoerOmregningEnsligDto::kompensasjonsgrad_ufoeretrygd_vedvirk)
        val inntekt_foer_ufoerhet_vedvirk = argument().select(UfoerOmregningEnsligDto::inntekt_foer_ufoerhet_vedvirk)
        val oppjustert_inntekt_foer_ufoerhet_vedvirk =
            argument().select(UfoerOmregningEnsligDto::oppjustert_inntekt_foer_ufoerhet_vedvirk)
        val tidligere_saerkullsbarn = argument().select(UfoerOmregningEnsligDto::tidligere_saerkullsbarn)
        val barn_overfoert_til_saerkullsbarn = argument().select(UfoerOmregningEnsligDto::barn_overfoert_til_saerkullsbarn)
        val barnetillegg_prosentsats_gradert_over_inntekt_foer_ufoer_vedvirk =
            argument().select(UfoerOmregningEnsligDto::barnetillegg_prosentsats_gradert_over_inntekt_foer_ufoer_vedvirk)
        val barnetillegg_gradert_over_inntekt_foer_ufoer_vedvirk =
            argument().select(UfoerOmregningEnsligDto::barnetillegg_gradert_over_inntekt_foer_ufoer_vedvirk)
        val barnetillegg_beloep_foer_reduksjon_vedvirk =
            argument().select(UfoerOmregningEnsligDto::barnetillegg_beloep_foer_reduksjon_vedvirk)
        val barnetillegg_saerkullsbarn_beloep_etter_reduksjon_vedvirk =
            argument().select(UfoerOmregningEnsligDto::barnetillegg_saerkullsbarn_beloep_etter_reduksjon_vedvirk)
        val barnetillegg_saerkullsbarn_inntekt_brukt_i_avkortning_vedvirk =
            argument().select(UfoerOmregningEnsligDto::barnetillegg_saerkullsbarn_inntekt_brukt_i_avkortning_vedvirk)
        val barnetillegg_saerkullsbarn_fribeloep_vedvirk =
            argument().select(UfoerOmregningEnsligDto::barnetillegg_saerkullsbarn_fribeloep_vedvirk)
        val barnetillegg_saerkullsbarn_inntektstak_vedvirk =
            argument().select(UfoerOmregningEnsligDto::barnetillegg_saerkullsbarn_inntektstak_vedvirk)


        outline {
            //TODO move texts into phrases
            title1 {
                text(
                    Bokmal to "Vedtak",
                    Nynorsk to "Vedtak",
            ...
            showIf(
                ...
                showIf(
                    not(har_barn_overfoert_til_saerkullsbarn) and not(barnetillegg_ikke_utbetalt_pga_tak)
                ) {
                    //infoBTSBInntekt_001
                    paragraph {
                        text(
            ...
            showIf(er_paragraf_3_2_samboer) {
                ...
                title1 {
                    textExpr(
                        Bokmal to "Rettigheter du kan ha som tidligere samboer med ".expr() + avdod_navn + "".expr(),
                        Nynorsk to "Rettar du kan ha som tidlegare sambuar med ".expr() + avdod_navn + "".expr(),
                        English to "Rights you may be entitled to as a former cohabitant with ".expr() + avdod_navn + "".expr()
                    )
                }
                // gjRettUTSamboer_001
                paragraph {
        ...
        //TODO vedlegg med conditions(inkluder vedlegg om...)
    }
}