...
export const merge: Action<LetterEditorState, [mergeId: ContentId, target: MergeTarget]> =
    produce((draft, mergeId, target) => {
        const editedLetter = draft.editedLetter
        const blocks = editedLetter.letter.blocks
        const prevContentSameBlock = blocks[mergeId.blockId]?.content[mergeId.contentId - 1]
        ...
        if ("itemId" in mergeId) {
        ...
        } else if (target === MergeTarget.PREVIOUS && prevContentSameBlock?.type === ITEM_LIST) {
            ...
            const lastItemId = prevContentSameBlock.items.length - 1
            const lastItem = prevContentSameBlock.items[lastItemId]
            const lastContentOfLastItem = lastItem.content[lastItem.content.length - 1]

            // Steal focus before we modify
            if (lastContentOfLastItem.type === LITERAL) {
                draft.stealFocus[mergeId.blockId] = {
            ...
            const textContentAfterList = content.splice(mergeId.contentId, nonTextContentRelativeIndex !== -1 ? nonTextContentRelativeIndex : content.length).filter(isTextContent)
            ...
            if (first != null && second != null) {
                if (isEmptyBlock(first)) {
                    ...
                    if (lastContentOfFirst.type === LITERAL) {
                        draft.stealFocus[firstId] = {contentId: first.content.length - 1, startOffset: lastContentOfFirst.text.length}
                    } else {
                        draft.stealFocus[firstId] = {contentId: first.content.length, startOffset: 0}
                    }
