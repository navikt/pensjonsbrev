...
const ScalarEditor: FC<ScalarEditorProps> = ({spec, value, updateValue}: ScalarEditorProps) => {
    switch (spec.kind) {
        case "NUMBER":
            return <input type="number" step={1} value={value as number} onChange={e => updateValue(e.target.value)}/>
        case "DOUBLE":
            return <input type="number" step={0.1} value={value as number} onChange={e => updateValue(e.target.value)}/>
        case "STRING":
            return <input type="text" value={value as string} onChange={e => updateValue(e.target.value)}/>
        case "BOOLEAN":
            return <input type="checkbox" checked={value as boolean} onChange={e => updateValue(e.target.checked)}/>
        case "DATE":
            return <input type="date" value={value as string} onChange={e => updateValue(e.target.value)}/>
    }
}
