...
import {AnyBlock, Item, ItemList, ParagraphBlock, TextContent} from "../../../../src/lib/model/api"
    ...
    describe('at literal', () => {
        describe("next", () => {
            test("the specified blocks are merged and the number of blocks reduced", () => {
                ...
                const result = Actions.merge(state, {blockId: 0, contentId: 0}, MergeTarget.NEXT)
            ...
            test("merge is ignored if the specified block is the last", () => {
                ...
                const result = Actions.merge(state, {blockId: 1, contentId: 0}, MergeTarget.NEXT)
            ...
            test("the content of the next block is added to the end of the specified", () => {
                ...
                expect(select<ParagraphBlock>(result, mergeId).content)
                    .toEqual(select<ParagraphBlock>(state, mergeId).content.concat(select<ParagraphBlock>(state, {blockId: mergeId.blockId + 1}).content))
            })

            test("adjoining literal content in merging blocks are joined", () => {
                const state = letter(paragraph(variable("var1"), literal("lit1")), paragraph(literal("lit2"), variable("var2")))
                const mergeId = {blockId: 0}
                ...
                expect(select<TextContent>(result, {...mergeId, contentId: 1}).text).toEqual("lit1lit2")
            })
            ...
            test("id of next block is kept if the specified is empty", () => {
                ...
                expect(select<AnyBlock>(result, mergeId).id).toEqual(select<AnyBlock>(state, {blockId: mergeId.blockId + 1}).id)
            })
        })
        ...
        describe("previous", () => {
            ...
            test("merge is ignored if the specified block is the first", () => {
                ...
                const result = Actions.merge(state, {blockId: 0, contentId: 0}, MergeTarget.PREVIOUS)
                expect(result).toBe(state)
            })
            ...
            test("the content of the specified block is added to the end of the previous", () => {
                ...
                const result = Actions.merge(state, {blockId: 1, contentId: 0}, MergeTarget.PREVIOUS)
                expect(select<ParagraphBlock>(result, {blockId: 0}).content)
                    .toEqual(select<ParagraphBlock>(state, {blockId: 0}).content.concat(select<ParagraphBlock>(state, {blockId: 1}).content))
            })
            ...
            test("adjoining literal content in merging blocks are joined", () => {
                ...
                // assertions
                expect(select<ParagraphBlock>(result, {blockId: 0}).content).toHaveLength(3)
                expect(select<TextContent>(result, {blockId: 0, contentId: 1}).text)
                    .toEqual("lit1lit2")
            })
            ...
            test("id of specified block is kept if the previous is empty", () => {
                const state = letter(paragraph(), paragraph(literal("p2")))
                const result = Actions.merge(state, {blockId: 1, contentId: 0}, MergeTarget.PREVIOUS)
                expect(select<AnyBlock>(result, {blockId: 0}).id).toEqual(select<AnyBlock>(state, {blockId: 1}).id)
            })

            test("id of previous block is kept if the specified is empty", () => {
                const state = letter(paragraph(literal("p1")), paragraph())
                const result = Actions.merge(state, {blockId: 1, contentId: 0}, MergeTarget.PREVIOUS)
                expect(select<AnyBlock>(result, {blockId: 0}).id).toEqual(select<AnyBlock>(state, {blockId: 0}).id)
            })
            ...
            describe('previous content in same block is itemList', () => {
                const mergeId = {blockId: 1, contentId: 1}
                ...
                test('does not merge with previous block', () => {
                    expect(result.editedLetter.letter.blocks).toHaveLength(withContentAfterList.editedLetter.letter.blocks.length)
                    expect(result.editedLetter.letter.blocks[mergeId.blockId - 1]).toBe(withContentAfterList.editedLetter.letter.blocks[mergeId.blockId - 1])
                })

                test('textcontent should be added to the last item', () => {
                    const itemList = select<ItemList>(result, {blockId: mergeId.blockId, contentId: mergeId.contentId - 1})
                    expect(itemList.items).toHaveLength(1)
                    expect(itemList.items[0].content[0].text).toEqual("Det blir content 1")
                    expect(itemList.items[0].content[1]).toBe(select(withContentAfterList, {...mergeId, contentId: mergeId.contentId + 1}))
                })

                test('should only merge textcontent into the last item', () => {
                    expect(select(result, mergeId)).toBe(select(withContentAfterList, {...mergeId, contentId: 3}))
                })

                test('focus should be stolen to the end of the last item', () => {
                    expect(result.stealFocus[mergeId.blockId]).toEqual({contentId: mergeId.contentId - 1, startOffset: "Det blir ".length, item: {id: 0, contentId: 0}})
                })
            })
        })
    })
    ...
    describe('at itemList', () => {
        ...
        describe("with next", () => {
            test("the specified items are merged and the number of items reduced", () => {
                ...
                const result = Actions.merge(state, {blockId: 0, contentId: 0, itemId: 0, itemContentId: 0}, MergeTarget.NEXT)
            ...
            test("merge is ignored if the specified item is the last", () => {
                const state = letter(paragraph(itemList(item(literal("lit1")), item(literal("lit2")))))
                const result = Actions.merge(state, {blockId: 0, contentId: 0, itemId: 1, itemContentId: 0}, MergeTarget.NEXT)
                expect(result).toBe(state)
            })
            ...
            test("the content of the next item is added to the end of the specified", () => {
                ...
                const mergeId = {blockId: 0, contentId: 0, itemId: 0}

                const result = Actions.merge(state, mergeId, MergeTarget.NEXT)

                expect(select<Item>(result, mergeId).content).toEqual(select<Item>(state, mergeId).content.concat(select<Item>(state, {...mergeId, itemId: 1}).content))
            })

            describe("adjoining literals", () => {
                const state = letter(paragraph(itemList(item(variable("var1"), literal("lit1")), item(literal("lit2"), variable("var2")))))
                const mergeId = {blockId: 0, contentId: 0, itemId: 0}
            ...
            describe('the next item is empty', () => {
                ...
                test('then the focus is stolen to the end of currrent', () => {
                    ...
                    expect(result.stealFocus[mergeId.blockId]).toEqual({
                        contentId: mergeId.contentId,
                        startOffset: item.content[item.content.length - 1].text.length,
            ...
            describe('the current item is empty', () => {
                ...
                test('then the current item is replaced with the next', () => {
                    expect(select<Item>(result, mergeId)).toBe(select<Item>(state, {...mergeId, itemId: mergeId.itemId + 1}))
                    expect(select<ItemList>(result, {blockId: 0, contentId: 0}).items).toHaveLength(1)
                })
        ...
        describe("with previous", () => {
            ...
            test("merge is ignored if the specified item is the first", () => {
                const state = letter(paragraph(itemList(item(literal("lit1")), item(literal("lit2")))))
                const result = Actions.merge(state, {blockId: 0, contentId: 0, itemId: 0}, MergeTarget.PREVIOUS)
            ...
            test("the content of the specified item is added to the end of the previous", () => {
                ...
                const mergeId = {blockId: 0, contentId: 0, itemId: 1}
                const result = Actions.merge(state, mergeId, MergeTarget.PREVIOUS)
                expect(select<Item>(result, {...mergeId, itemId: mergeId.itemId - 1}).content)
                    .toEqual(select<Item>(state, {...mergeId, itemId: mergeId.itemId - 1}).content.concat(select<Item>(state, mergeId).content))
            })
            ...
            test("adjoining literal content in merging items are joined", () => {
                ...
                const mergedId = {...mergeId, itemId: mergeId.itemId - 1}
                expect(select<Item>(result, mergedId).content).toHaveLength(3)
                expect(select<TextContent>(result, {...mergedId, itemContentId: 1}).text).toEqual("lit1lit2")
            })
            ...
            describe('the current item is empty', () => {
                ...
                test('then the current is removed', () => {
                    expect(select<ItemList>(result, {blockId: 0, contentId: 0}).items).toHaveLength(1)
                    expect(select<Item>(result, {blockId: 0, contentId: 0, itemId: 0})).toBe(select<Item>(state, {blockId: 0, contentId: 0, itemId: 0}))
                    expect(select<Item>(result, mergeId)).toBeUndefined()
                })

                test('then the focus is stolen to the end', () => {
                    const prevItem = select<Item>(state, {...mergeId, itemId: mergeId.itemId - 1})
                    expect(result.stealFocus[mergeId.blockId]).toEqual({
                        contentId: mergeId.contentId,
                        startOffset: prevItem.content[prevItem.content.length - 1].text.length,
                        item: {id: mergeId.itemId - 1, contentId: prevItem.content.length - 1},
                    })
                })
            })
        })
