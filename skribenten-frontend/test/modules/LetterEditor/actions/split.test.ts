...
import {Content, ItemList, LITERAL, ParagraphBlock, TextContent} from "../../../../src/lib/model/api"
...
describe('LetterEditorActions.split', () => {
    describe('at literal', () => {
        test("specified block is split at contentId and offset", () => {
            ...
            expect(select<Content>(result, {blockId: 1, contentId: 1})).toStrictEqual(select<Content>(state, {blockId: 0, contentId: 2}))
    ...
    describe('at itemList', () => {
        test('specified item is split at contentId and offset', () => {
            ...
            expect(splitItem.content[splitId.itemContentId].text).toEqual(original.items[splitId.itemId].content[splitId.itemContentId].text.substring(0, offset))
            ...
            expect(newItem.content[0].text).toEqual(original.items[splitId.itemId].content[splitId.itemContentId].text.substring(offset))

            // focus is stolen to beginning of new item
            expect(result.stealFocus).toEqual({[splitId.blockId]: {contentId: splitId.contentId, startOffset: 0, item: {id: splitId.itemId + 1, contentId: 0}}})
        })
        ...
        test('does not split so that we get multiple empty items', () => {
            ...
            expect(Actions.split(state, {blockId: 0, contentId: 0, itemId: 0,  itemContentId: 0}, "item1".length)).toBe(state)
