...
val vedleggInformasjonOmMedlemskapOgHelserettigheterEOES =
    createAttachment<LangBokmalNynorskEnglish, EmptyVedleggData>(
        ...
        paragraph {
            text(
                bokmal { +"Selv om du betaler skatt til Norge, er du ikke automatisk medlem i folketrygden. Du kan lese mer om dette på ${Constants.MEDLEMSKAP_URL}." },
                nynorsk { +"Sjølv om du betalar skatt til Noreg, er du ikkje automatisk medlem i folketrygda. Du kan lese meir om dette på ${Constants.MEDLEMSKAP_URL}" },
                english { +"Even if you pay tax to Norway, you are not automatically a member of the National Insurance Scheme. You can read more about this at ${Constants.MEDLEMSKAP_URL}." }
            )
        }
        ...
        paragraph {
            text(
                bokmal { +"For å ha rett til helsetjenester må du bruke blankett E121/S1. Blanketten får du hos ${Constants.HELFO}." },
                nynorsk { +"For å ha rett til helsetenester må du bruke blankett E121/S1. Blanketten får du hjå ${Constants.HELFO}." },
                english { +"To be entitled to health services, you must use form E121/S1. The form can be obtained at ${Constants.HELFO}." }
            )
        }
        paragraph {
            text(
                bokmal { +"Du må kontakte ${Constants.HELFO} dersom du har spørsmål om helsetrygdkort, blankett E 121/S1 og helsetjenester mens du oppholder deg i utlandet." },
                nynorsk { +"Du må kontakte ${Constants.HELFO} dersom du har spørsmål om helsetrygdkort, blankett E 121/S1 og helsetenester medan du oppheld deg i utlandet." },
                english { +"You must contact ${Constants.HELFO} if you have any questions about health benefit cards, form E 121/S1 and health services while you are abroad." }
            )
        }
        paragraph {
            list {
                item {
                    text(
                        bokmal { +"Internett: ${Constants.HELSENORGE}." },
                        nynorsk { +"Internett: ${Constants.HELSENORGE}." },
                        english { +"Internet: ${Constants.HELSENORGE}." }
                    )
                }
                item {
                    text(
                        bokmal { +"Telefon: ${Constants.TELEFON_HELSE}" },
                        nynorsk { +"Telefon: ${Constants.TELEFON_HELSE}." },
                        english { +"Telephone: ${Constants.TELEFON_HELSE}" }
                    )
                }
            }
        }
        ...
        paragraph {
            text(
                bokmal { +"Du kan, på nærmere bestemte vilkår, bli frivillig medlem i folketrygdens helsedel dersom du søker om det. Du kan lese mer om dette på ${Constants.NAV_URL}." },
                nynorsk { +"Du kan, på nærare fastsette vilkår, bli frivillig medlem i helsedelen av folketrygda dersom du søkjer om det. Du kan lese meir om dette på ${Constants.NAV_URL}." },
                english { +"You may, on further specified terms, become a voluntary member of the National Insurance Scheme if you apply for it. You can read more about this at ${Constants.NAV_URL}." }
            )
        }
