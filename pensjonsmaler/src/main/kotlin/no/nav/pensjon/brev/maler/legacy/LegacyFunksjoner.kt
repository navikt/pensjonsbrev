...
fun Expression<PE>.ut_tbu056v() =
    (
        pebrevkode().equalTo("PE_UT_04_102")
            ...
            or (
                vedtaksdata_kravhode_kravarsaktype().equalTo("endret_inntekt")
                    and
                    vedtaksdata_beregningsdata_beregningufore_belopsendring_uforetrygdordineryk_belopgammelut().notEqualTo(
...
fun Expression<PE>.pe_ut_barnet_barna_felles_serkull(): Expression<String> {
    ...
    val erEttBarn =
        (
            barnetilleggfelles.antallbarnfelles_safe.ifNull(0).equalTo(1) and
                barnetilleggfelles.btfbnetto_safe.ifNull(0).equalTo(0)
        ) or
            (
    ...
    val erFlereBarn =
        (
            barnetilleggfelles.antallbarnfelles_safe.ifNull(0).greaterThan(1) and
                barnetilleggfelles.btfbnetto_safe.ifNull(0).equalTo(0)
        ) or
            (
