...
data class TBU2357_Generated(
    ...
    override fun OutlineOnlyScope<LangBokmalNynorskEnglish, Unit>.template() {
		...
		paragraph {
			textExpr (
				Bokmal to "Vi har lagt til grunn at du framover skal ha en inntekt på ".expr() + pe.ut_inntektsgrense_faktisk_minus_60000().format(
                    true
                ) + " kroner per år. Du kan i tillegg ha en årlig inntekt på 60 000 kroner, uten at uføretrygden din blir redusert. Inntektsgrensen din blir derfor " + pe.ut_inntektsgrense_faktisk().format(
                    true
                ) + " kroner.",
				Nynorsk to "Vi har lagt til grunn at du framover skal ha ei inntekt på ".expr() + pe.ut_inntektsgrense_faktisk_minus_60000().format(
                    true
                ) + " kroner per år. Du kan i tillegg ha ei årleg inntekt på 60 000 kroner utan at uføretrygda di blir redusert. Inntektsgrensa di blir derfor " + pe.ut_inntektsgrense_faktisk().format(
                    true
                ) + " kroner.",
