...
object FormatBorMedSivilstandTabell : LocalizedFormatter<BorMedSivilstand>() {
    override fun apply(
        ...
        when (first) {
            ...
            BorMedSivilstand.PARTNER, BorMedSivilstand.PARTNER_LEVER_ADSKILT ->
                when (second) {
                    Bokmal -> "Partner"
                    Nynorsk -> "Partnar"
                    English -> "Partnership"
...
object SivilstandEpsBestemt : LocalizedFormatter<Sivilstand>() {
    override fun apply(
        first: Sivilstand,
        second: Language,
    ): String = sivilstand(first, second, true)
...
object BorMedSivilstandUbestemt : LocalizedFormatter<BorMedSivilstand>() {
    override fun apply(
        first: BorMedSivilstand,
        second: Language,
    ): String = borMedSivilstand(first, second, false)
...
object BorMedSivilstandBestemt : LocalizedFormatter<BorMedSivilstand>() {
    override fun apply(
        first: BorMedSivilstand,
        second: Language,
    ): String = borMedSivilstand(first, second, true)

    override fun stableHashCode(): Int = "BorMedSivilstandBestemt".hashCode()
}

private fun borMedSivilstand(
    sivilstand: BorMedSivilstand,
    language: Language,
    bestemtForm: Boolean,
): String =
    when (sivilstand) {
        ...
        BorMedSivilstand.SAMBOER1_5, BorMedSivilstand.SAMBOER3_2 ->
            when (language) {
                Bokmal -> if (bestemtForm) "samboeren" else "samboer"
                Nynorsk -> if (bestemtForm) "sambuaren" else "sambuar"
                English -> "cohabitant"
