...
object VarselSaksbehandlingstidAutoV2 : AutobrevTemplate<VarselSaksbehandlingstidAutoDto> {
    ...
    override val template = createTemplate(
        ...
        outline {
            ...
            paragraph {
                textExpr(
                    Bokmal to "Søknaden din blir behandlet så snart som mulig. Når søknaden er ferdig behandlet, får du et svar fra oss på ".expr() + quoted("Min side") +" på $NAV_URL. Du kan sjekke saksbehandlingstidene på $SAKSBEHANDLINGSTID_URL.",
                    Nynorsk to "Søknaden din vert handsama så snart som mogleg. Når søknaden er ferdig behandla, får du eit svar frå oss på ".expr() + quoted("Mi side") +" på $NAV_URL. Du kan sjekke saksbehandlingstidene på $SAKSBEHANDLINGSTID_URL.",
                    English to "Your application will be processed as soon as possible. When your application has been processed, you will receive a response from us on ".expr() + quoted("My Page") +" at $NAV_URL. You can check the processing times at $SAKSBEHANDLINGSTID_URL."
                )
            }
