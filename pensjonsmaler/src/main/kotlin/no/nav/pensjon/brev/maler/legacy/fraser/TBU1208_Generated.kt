...
data class TBU1208_Generated(
    ...
    override fun OutlineOnlyScope<LangBokmalNynorskEnglish, Unit>.template() {
		...
		paragraph {
			textExpr(
				Bokmal to "For deg utgjør kompensasjonsgraden ".expr() + pe.vedtaksdata_beregningsdata_beregningufore_beregningytelseskomp_uforetrygdordiner_avkortningsinformasjon_kompensasjonsgrad().format() + " prosent. Det er bare den delen av inntekten din som overstiger " + pe.ut_inntektsgrense_faktisk().format(
                    true
                ) + " kroner, som vi justerer uføretrygden din ut fra. Det betyr at et beløp som tilsvarer " + pe.vedtaksdata_beregningsdata_beregningufore_beregningytelseskomp_uforetrygdordiner_avkortningsinformasjon_kompensasjonsgrad().format() + " prosent av den inntekten du har over " + pe.ut_inntektsgrense_faktisk().format(
                    true
                ) + " kroner trekkes fra uføretrygden din.",
				Nynorsk to "For deg utgjer kompensasjonsgraden ".expr() + pe.vedtaksdata_beregningsdata_beregningufore_beregningytelseskomp_uforetrygdordiner_avkortningsinformasjon_kompensasjonsgrad().format() + " prosent. Det er berre den delen av inntekta di som overstig " + pe.ut_inntektsgrense_faktisk().format(
                    true
                ) + " kroner, som vi justerer uføretrygda di ut frå. Det betyr at eit beløp som svarer til " + pe.vedtaksdata_beregningsdata_beregningufore_beregningytelseskomp_uforetrygdordiner_avkortningsinformasjon_kompensasjonsgrad().format() + " prosent av inntekta du har over " + pe.ut_inntektsgrense_faktisk().format(
                    true
                ) + " kroner blir trekt frå uføretrygda di.",
				English to "It is only the part of your income that exceeds the income limit that will result in a reduction of your disability benefit. Your disability benefit is reduced by ".expr() + pe.vedtaksdata_beregningsdata_beregningufore_beregningytelseskomp_uforetrygdordiner_avkortningsinformasjon_kompensasjonsgrad().format() + " percent of income exceeding NOK " + pe.ut_inntektsgrense_faktisk().format(
                    true
