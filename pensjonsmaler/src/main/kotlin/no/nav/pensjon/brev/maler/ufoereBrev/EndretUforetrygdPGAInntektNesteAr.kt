...
object EndretUforetrygdPGAInntektNesteAr : AutobrevTemplate<EndretUTPgaInntektDtoV2> {
    ...
    override val template = createTemplate(
        ...
        outline {
                ...
                showIf(barnetilleggFB.inntektAnnenForelder.greaterThan(0) and barnetilleggFB.inntektBruker.greaterThan(0)) {
                    title1 {
                        text(
                            ...
                            nynorsk { + "Derfor oppjusterer vi inntekta di og annen forelder" },
                        )
                    }
                    paragraph {
                        text(
            ...
            paragraph {
                text(
                    bokmal { + "Den årlige inntekten vi vil bruke for deg er " + uforetrygd.inntektBruktIAvkortning.format(CurrencyFormat) +
                            " kroner. Det gir deg rett til en årlig utbetaling på " + totalNettoAr.format(CurrencyFormat) + " kroner. " },
                    nynorsk { + "Den årlege inntekta vi vil bruke for deg er " + uforetrygd.inntektBruktIAvkortning.format(CurrencyFormat) +
            ...
            paragraph {
                text(
                    bokmal { +  "Du finner fullstendige beregninger i vedlegget «Slik er uføretrygden din beregnet»." },
                    nynorsk { +  "Du finn fullstendige utrekningar i vedlegget «Slik er uføretrygda di rekna ut»." }
                )
            }
            paragraph {
                showIf(
            ...
            paragraph {
                text(
                    bokmal { +  "Du har rett til å se dokumentene i saken din. Se vedlegg «Dine rettigheter og plikter» for informasjon om hvordan du går fram./klage." },
