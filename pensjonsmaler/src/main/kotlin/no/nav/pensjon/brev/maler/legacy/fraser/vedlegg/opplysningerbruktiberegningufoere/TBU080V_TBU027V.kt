    ...
    override fun OutlineOnlyScope<LangBokmalNynorskEnglish, Unit>.template() {
        ...
        showIf((pe.pebrevkode().notEqualTo("PE_UT_07_100") and pe.pebrevkode().notEqualTo("PE_UT_05_100") and pe.pebrevkode().notEqualTo("PE_UT_04_300") and pe.pebrevkode().notEqualTo("PE_UT_14_300") and pe.pebrevkode().notEqualTo("PE_UT_04_103") and pe.vedtaksdata_kravhode_kravarsaktype().notEqualTo("soknad_bt") and pe.pebrevkode().notEqualTo("PE_UT_04_108") and pe.pebrevkode().notEqualTo("PE_UT_04_109") and pe.pebrevkode().notEqualTo("PE_UT_07_200") and pe.pebrevkode().notEqualTo("PE_UT_06_300"))){
            ...
            showIf((pe.vedtaksdata_beregningsdata_beregningufore_uforetrygdberegning_beregningsmetode().equalTo("eos") and not(pe.vedtaksbrev_vedtaksdata_vilkarsvedtak_vilkar_medlemskapforutettertrygdeavtaler_oppfyltvedsammenlegging()))){
                ...
                paragraph {
                    textExpr (
                        Bokmal to "Vi beregner uføretrygden din etter bestemmelsene i EØS-avtalen fordi det gir deg en høyere uføretrygd enn en beregning etter folketrygdlovens regler. Uføretrygden din er først beregnet etter samlet opptjening i Norge og andre EØS-land. Deretter er uføretrygden multiplisert med et forholdstall, som angir forholdet mellom faktisk opptjeningstid i Norge og samlet faktisk opptjening i Norge og andre EØS-land. Du kan se hvilke opplysninger vi har brukt i beregningen i tabellen ".expr() + quoted("Opplysninger vi har brukt i beregningen") +".",
                        Nynorsk to "Vi bereknar uføretrygda di etter reglane i EØS-avtalen fordi det gir deg ei høgare uføretrygd enn ei berekning etter reglane i folketrygdlova. Uføretrygda di er først berekna etter samla opptening i Noreg og andre EØS-land. Deretter er uføretrygda multiplisert med eit forholdstal som angir forholdet mellom faktisk oppteningstid i Noreg og samla faktisk opptening i Noreg og andre EØS-land. Du kan sjå kva opplysningar vi har brukt i berekninga i tabellen ".expr() + quoted("Opplysningar vi har brukt i berekninga") + ".",
                        English to "We have calculated your disability benefit in accordance with the provisions of the EEA agreement, because this will result in a higher benefit for you than if Norwegian National Insurance provisions were applied. Your disability benefit has first been calculated on the basis of your total pensionable income in Norway and other EEA countries. Then, your disability benefit has been multiplied by the ratio between your actual insurance periods in Norway and total actual insurance periods in Norway and other EEA countries. You can see the data we have used in our calculations in the table ".expr() + quoted("Data used in calculations") +".",
                    )
                }
            }
            ...
            showIf((pe.vedtaksbrev_vedtaksdata_vilkarsvedtak_vilkar_medlemskapforutettertrygdeavtaler_oppfyltvedsammenlegging() and (pe.vedtaksdata_beregningsdata_beregningufore_uforetrygdberegning_beregningsmetode().equalTo("eos") or pe.vedtaksdata_beregningsdata_beregningufore_uforetrygdberegning_beregningsmetode().equalTo("nordisk")))){
                ...
                paragraph {
                    textExpr (
                        Bokmal to "Vi beregner uføretrygden din etter bestemmelsene i EØS-avtalen fordi vilkårene for rett til uføretrygd er oppfylt ved sammenlegging av perioder med opptjening i Norge og andre EØS-land. Uføretrygden din er først beregnet etter samlet opptjening i Norge og andre EØS-land. Deretter er uføretrygden multiplisert med et forholdstall, som angir forholdet mellom faktisk opptjeningstid i Norge og samlet faktisk opptjening i Norge og andre EØS-land. Du kan se hvilke opplysninger vi har brukt i beregningen i tabellen ".expr() + quoted("Opplysninger vi har brukt i beregningen") +".",
                        Nynorsk to "Vi bereknar uføretrygda di etter reglane i EØS-avtalen fordi vilkåra for rett til uføretrygd er oppfylte når vi legg saman periodar med opptening i Noreg og andre EØS-land. Uføretrygda di er først berekna etter samla opptening i Noreg og andre EØS-land. Deretter er uføretrygda multiplisert med eit forholdstal som angir forholdet mellom faktisk oppteningstid i Noreg og samla faktisk opptening i Noreg og andre EØS-land. Du kan sjå kva opplysningar vi har brukt i berekninga i tabellen ".expr() + quoted("Opplysningar vi har brukt i berekninga") + ".",
            ...
            showIf((pe.vedtaksdata_beregningsdata_beregningufore_uforetrygdberegning_beregningsmetode().notEqualTo("eos") and pe.vedtaksdata_beregningsdata_beregningufore_uforetrygdberegning_beregningsmetode().notEqualTo("folketrygd") and pe.vedtaksdata_beregningsdata_beregningufore_uforetrygdberegning_beregningsmetode().notEqualTo("nordisk") and pe.vedtaksbrev_vedtaksdata_vilkarsvedtak_vilkar_medlemskapforutettertrygdeavtaler_oppfyltvedsammenlegging())){
                ...
                paragraph {
                    textExpr (
                        Bokmal to "Vi beregner uføretrygden din etter bestemmelsene i trygdeavtalen med ".expr() + pe.grunnlag_persongrunnlagsliste_trygdeavtaler_avtaleland() + " fordi vilkårene for rett til uføretrygd er oppfylt ved sammenlegging av opptjeningstid i Norge og " + pe.grunnlag_persongrunnlagsliste_trygdeavtaler_avtaleland() + ". Uføretrygden din er først beregnet etter samlet opptjening i Norge og " + pe.grunnlag_persongrunnlagsliste_trygdeavtaler_avtaleland() + ". Deretter er uføretrygden multiplisert med et forholdstall, som angir forholdet mellom faktisk opptjeningstid i Norge og samlet faktisk opptjening i Norge og " + pe.grunnlag_persongrunnlagsliste_trygdeavtaler_avtaleland() + ". Du kan se hvilke opplysninger vi har brukt i beregningen i tabellen " + quoted("Opplysninger vi har brukt i beregningen") +".",
            ...
            showIf((pe.vedtaksdata_beregningsdata_beregningufore_uforetrygdberegning_beregningsmetode().equalTo("folketrygd") and pe.vedtaksdata_kravhode_vurderetrygdeavtale())){
                ...
                paragraph {
                    textExpr (
                        Bokmal to "Vi beregner uføretrygden din etter folketrygdlovens regler fordi dette gir deg en høyere beregning enn etter trygdeavtale. Uføretrygden din er beregnet etter samlet opptjening i Norge. Du kan se hvilke opplysninger vi har brukt i beregningen i tabellen ".expr() + quoted("Opplysninger vi har brukt i beregningen") +".",
                        Nynorsk to "Vi bereknar uføretrygda di etter reglane i folketrygdlova fordi dette gir deg ei høgare berekning enn etter trygdeavtale. Uføretrygda di er berekna etter samla opptening i Noreg. Du kan sjå kva opplysningar vi har brukt i berekninga i tabellen ".expr() + quoted("Opplysningar vi har brukt i berekninga") +".",
