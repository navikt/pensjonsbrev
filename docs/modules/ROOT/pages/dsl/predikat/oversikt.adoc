...
|`greaterThan()`
|Større enn
|`uforegrad.greaterThan(50.expr())`

|`lessThan()`
|Mindre enn
|`alder.lessThan(67.expr())`
...
----
// Sjekk uføregrad
showIf(uforegrad.greaterThan(50.expr())) {
...
showIf(harBarnetillegg and borINorge) {
    text(bokmal { +"Du får norsk barnetillegg." })
}
...
showIf(barn.isNotEmpty()) {
    text(bokmal { +"Du har registrerte barn." })
...
showIf(perioder.size().greaterThan(1.expr())) {
