...
| Ikke i bruk. CPU-struping kan ikke måles så lenge vi ikke setter `limits.cpu`, jf. <<ikke-alarmer, Bevisste ikke-alarmer>>.
...
[NOTE]
====
Alle nedstrøms HTTP-kall instrumenteres av `services/HttpClientMetrics.kt` og eksponeres som `skribenten_http_client_requests_seconds`, med labels `downstream_host` (tjenesten, fra kallets URL), `route` (endepunktet, satt av kallstedet) og `outcome` (`success`/`client_error`/`server_error`/`timeout`/`connection_error`/`exception`).

Retries telles separat per forsøk, ikke bare det endelige utfallet. `NaisLeaderService` sitt kall mot nais' lokale leader-election-sidecar (`downstream_host="localhost"`) er ekskludert i PromQL-en under, siden det ikke er en ekte nedstrøms tjeneste.
====

[NOTE]
====
Valkey-cachen (`common/Cache.kt`) instrumenteres med to metrikker.

* `skribenten_cache_requests_total`, med labels `omraade` (nøkkelens prefiks, satt av `Cacheomraade.prefix`, f.eks. `Norg`, `SamhandlerEnkel`, `PensjonPersondata`) og `result` (`hit`/`miss`). Gir treffrate per område:
+
[source]
----
sum by (omraade) (rate(skribenten_cache_requests_total{app="skribenten-backend", result="hit"}[1h]))
/
sum by (omraade) (rate(skribenten_cache_requests_total{app="skribenten-backend"}[1h]))
----
* `skribenten_cache_operations_seconds`, med labels `operation` (`read`/`update`) og `outcome` (`success`/`error`). Måler selve Valkey-kallet, uavhengig av `omraade`, og dekker to ting i én metrikk: latens (`_sum`/`_bucket`) og feilrate mot Valkey (`_count` filtrert på `outcome="error"`).

Et leseforsøk som feiler mot Valkey (f.eks. ved en driftsforstyrrelse) gir `outcome="error"` på `skribenten_cache_operations_seconds`, men telles bevisst *ikke* som `miss` i `skribenten_cache_requests_total` - ellers ville en Valkey-hendelse feilaktig se ut som en for kort TTL.

Begge metrikkene er satt sammen til et dashbord for å vurdere TTL-valgene per cacheområde: link:{attachmentsdir}/cache-ttl-dashboard.json[cache-ttl-dashboard.json].
====
...
| `SkribentenNedstroemsHoeyFeilrate`
| Andel kall per `downstream_host` som ender i `server_error`/`timeout`/`connection_error`/`exception` over 5 % *og* minst 3 feil, i 10 min
| `critical`
| Skiller «PEN er nede» fra «SAF er nede» osv. uten å måtte lese logger. Terskelen gjenbruker samme nivå som `SkribentenHoeyFeilrate` siden konsekvensen er lik: saksbehandlere blokkeres i handlinger som avhenger av den tjenesten.

| `SkribentenNedstroemsHoeyLatens`
| p95-latens per `downstream_host` over 10s, i 15 min
| `warning`
| Fanger opp treg nedstrøms tjeneste før den ender som timeout og dermed som feilrate. Terskelen er satt lavt sammenlignet med `requestTimeout` (60s mot brevbaker) med vilje: den skal varsle *før* kall begynner å timeoute, ikke samtidig med det.
|===
...
| `up == 0` i 10 min, unntatt poder på en node som ikke er `Ready`
| `warning`
...
Risikoen ved å stå blind over tid håndteres av en dødmannsknapp per app, ikke av alvorlighetsgraden her — se hull 9.
...
CPU-strupning i pdf-bygger::
Typst er CPU-bundet, så strupning ville vært et reelt problem — men det *kan ikke oppstå* slik appene er satt opp.
CFS-strupning forutsetter en CPU-kvote på containeren, og kvoten kommer kun av `resources.limits.cpu`.
Ingen av applikasjonene våre setter den, Nais setter ingen default for CPU (i motsetning til minne), og https://doc.nais.io/workloads/application/reference/application-spec/[Nais fraråder den aktivt].
Uten kvote finnes `container_cpu_cfs_throttled_periods_total` ikke for containeren, og en alarm på den ville aldri utløst.
+
Vi hadde en slik alarm, `PdfByggerCpuStrupes`, frem til den ble avdekket som varig stum under gjennomgangen av alarmreglene mot prod.
Symptomet den var ment å fange — treg brevproduksjon — dekkes allerede av `PdfByggerTregProduksjon`, jf. <<prinsipper, prinsipp 1>>.
+
Skulle vi senere sette et CPU-limit, må denne vurderingen gjøres på nytt.
Da må strupning måles som *andel strupede perioder* (`container_cpu_cfs_throttled_periods_total / container_cpu_cfs_periods_total`), ikke som strupede sekunder: `container_cpu_cfs_throttled_seconds_total` summerer på tvers av kjerner, slik at tallet vokser med antall kjerner uten at strupningen er verre.
...
| 9
| BrevbakerKoeBunnerSegOpp: terskelen på 10 er ikke kalibrert mot prod.
