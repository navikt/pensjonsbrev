...
Denne siden er fasit: endrer vi en alarm, oppdateres tabellene her i samme PR.
...
Vi ønsket å gjøre unntak der metrikker ikke finnes, og der loggmeldingen er stabil og entydig:
...
* `Alle toggles må være definert i Unleash` — en brevmal har en feature toggle som ikke finnes i Unleash, slik at malen blir stille utilgjengelig.

[IMPORTANT]
====
*Ingen av disse er implementert, og de kan ikke implementeres slik alarmene våre er bygget.*
...
Konsekvensen er reell: `Error executing one-shot job` var den eneste dekningen for en engangsjobb som feiler stille, og `Alle toggles må være definert i Unleash` den eneste for en brevmal som blir utilgjengelig uten at noe feiler.
Begge er nå udekket. Se <<hull, Kjente hull>>.

Den varige løsningen for de fleste av disse er uansett en metrikk framfor en loggalarm, jf. hull 3 og 6.
====
...
Den kan *ikke* se om metrikk- eller labelnavn faktisk finnes; det avdekkes først når regelen kjører mot Mimir.

=== Alarmene gjelder kun prod

Alarmene rulles ut til `prod-gcp` alene. Vi ønsker ikke varsler fra dev, der applikasjonene rutinemessig er nede eller halvferdige.
Konsekvensen er at appnavn står hardkodet i reglene i stedet for å komme fra en variabel.

[WARNING]
====
*Alarmfilene må rulles ut fra en workflow uten `VARS`/`VAR`, og det er ikke valgfritt.*

`nais/deploy` kjører Handlebars (`aymerick/raymond`) over alle ressursfiler, og Handlebars bruker samme `{{ }}`-syntaks som Go-templatene Prometheus trenger i annotasjonene.
Templating hoppes over helt når variabelkonteksten er tom (`templatedFile` i `pkg/deployclient/template.go` returnerer uendret innhold når `len(ctx) == 0`), så en workflow uten variabler lar Go-templatene stå i fred.

Legger noen til `VARS` eller `VAR` senere, brekker alarmene på to ulike måter — og den ene er stille:

Uttrykk med pipe, f.eks. `humanizePercentage` eller `printf`::
*Parse-feil, og deployet stopper.* Raymond tolker pipe-tegnet som block params. Høylytt og lett å oppdage.

Rene labelreferanser, f.eks. `$labels.pod`::
*Tom streng, ingen feil.* Alarmen fortsetter å virke, men teksten vi får i Slack mangler nettopp den informasjonen som gjør den handlingsrettet.

Den stille varianten er den farlige, og den rammer verst der det gjør mest vondt: `plattform-alerts.yaml` har ingen pipes i det hele tatt, men 13 labelreferanser.
Den ville altså blitt rullet ut helt uten feilmelding, med pod-, container- og deployment-navn strøket fra samtlige alarmtekster.

Merk at `nais validate` *ikke* oppfører seg som deploy her: CLI-en advarer om manglende variabler og setter inn en placeholder-verdi.
Det er en hjelp ved lokal validering, men det betyr at en `nais validate` som ser fin ut ikke er noen garanti for hva deploy faktisk gjør.
...
Definert i `pensjon/brevbaker/.nais/alerts.yaml`.
...
| `warning`
| Requests fullføres ikke. Typisk tegn på at pdf-bygger henger og at retry-logikken holder tråder opptatt. `min_over_time` skiller vedvarende oppbygging fra normale topper, som drenerer mellom hver spike. *Terskelen på 10 er ikke kalibrert mot prod* og bør justeres når vi har sett den over en batch.

| `BrevbakerIngenBrevProdusert`
| `pensjon_brevbaker_letter_request_count_total` under 1 på 2 timer, i kontortid på ukedager
| `critical`
| Fanger den stille feilen: alt ser friskt ut, men ingen bestiller brev fordi Pesys ikke når fram. Ingen andre alarmer fanger dette, siden feilrate og latens begge er tomme når det ikke kommer trafikk.
|===
...
Planen var å alarmere på et *forventet batchvindu*, men de store brevbestillingene har ingen fast frekvens: de kjøres til ulike formål med hver sin frekvens, i tillegg til ad hoc-kjøringer.
Det finnes derfor ikke noe vindu vi kan si at «her skal det alltid komme brev».

I stedet alarmerer vi på total stillhet i kontortid, der det alltid er baseline-trafikk fra Skribenten i tillegg til autobrev.
Det gir dårligere dekning — en batch som ikke kjørte om natten fanges ikke — men er den sterkeste regelen datagrunnlaget tillater.
...
* *Brå fall i `pensjon_brevbaker_letter_request_count_total` for en enkelt `brevkode`* (`warning`). Kan bety at én brevmal er ødelagt selv om totalen ser normal ut. Krever at vi har nok historikk til å definere «normalt» per brevkode; vurderes på nytt når vi har det.
* *Logg: `Alle toggles må være definert i Unleash`* (`warning`). En brevmal med en toggle som ikke finnes i Unleash blir *stille utilgjengelig* — ingenting feiler, brevet kan bare ikke bestilles. Kan ikke implementeres som `PrometheusRule`, se <<hull, Kjente hull>>.
...
* *Logg: `typst command failed`* (`warning`). Skulle skille malfeil (én brevmal kompilerer ikke) fra infrastrukturfeil. Kan ikke implementeres som `PrometheusRule`, og bør uansett erstattes av en metrikk, se hull 3 under <<hull, Kjente hull>>.
...
* *`/isReady` returnerer 503 på flere poder* (`critical`). Ruten instrumenteres ikke lenger, jf. <<implementasjon, helsesjekkfiltreringen>>, så alarmen kan ikke skrives som beskrevet. Den er heller ikke lenger nødvendig: en pod som feiler readiness tas ut av rotasjon og fanges av `IngenTilgjengeligePoder` og `FaerreTilgjengeligePoderEnnOensket`, som dekker det samme utfallet bedre.
* *Feil mot en enkelt nedstrøms tjeneste* (`warning`). Skal identifisere *hvilken* tjeneste som feiler. Krever instrumentering av HTTP-klientene, se hull 1 under <<hull, Kjente hull>>.
* *Logg: `Error executing one-shot job`* (`warning`). En feilet engangsjobb stopper *ikke* oppstarten — jobbene kjøres i en egen coroutine etter `ServerReady` med forsinkelse, og fullføring registreres kun i `OneShotJobTable` ved suksess. En jobb kan derfor feile ved hver oppstart uten at noe annet indikerer det. Dette var eneste signal, og er nå udekket, se <<hull, Kjente hull>>.
* *Databasetilkoblinger nær maks* (`warning`). Tilkoblingslekkasje ender i total utilgjengelighet. Krever at connection pool-metrikker eksponeres, se hull 5 under <<hull, Kjente hull>>.
...
Podrestarter dekkes av `ContainerCrashlooper` og `PodOOMKilled` blant <<felles, de felles plattformalarmene>>, og er derfor ikke gjentatt her.
...
[NOTE]
====
`MinnebrukNaerGrensen` må sammenligne to metrikker som bærer ulike hjelpelabels, og bruker derfor `on(namespace, pod, container)` i divisjonen.
Uten den eksplisitte listen finner Prometheus ingen matchende par, og regelen blir stille i stedet for å feile.
====
...
Ingen av de gjenstående hullene blokkerer alarmene som er implementert, men hull 6 blokkerer *fire alarmer vi ønsket oss* og som derfor ikke finnes.
De to opprinnelige blokkerende hullene er lukket: latensmetrikken eksporteres nå som histogram, og skribenten-web eksponerer metrikker.
Resten forbedrer i hovedsak *feilsøkingen* når en alarm har gått — det gjør oss raskere til å finne årsaken, ikke flinkere til å oppdage at noe er galt.
...
| Legg til teller for typst-resultat (suksess/kompileringsfeil/eksekveringsfeil) og timer for kompileringstid. Lukker også deler av hull 6.
| Middels
...
| Enten sett opp en `LokiRule`-pipeline, eller — som regel bedre — erstatt hver enkelt med en metrikk. En teller for feilede engangsjobber og en for maler med udefinert toggle lukker de to alvorligste.
| *Høy*

| 7
| Alarmene bruker `kube_*`- og `nginx_ingress_controller_requests`-labels som ikke er verifisert mot vår egen Mimir. `promtool` validerer syntaks, ikke at metrikk- og labelnavn finnes. En regel med feil labelnavn blir *stille* — den feiler ikke, den utløser bare aldri.
