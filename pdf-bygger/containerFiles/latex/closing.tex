%----------------------------------------
%       Closing/last page of letter
%----------------------------------------

% Closing page text
\newcommand{\closing}{
    \addpenalty{9000} % Penalty for putting minipage on new page
    \addvspace{24pt minus 10pt} % TODO gjennomgang av design
    \begin{minipage}{\textwidth}
        \standardtext
        \sloppy % no word-breaking
        \raggedright % no word spacing
        \raggedbottom % no line stretching to fit page
        \frenchspacing % no double space after ended sentence
        \feltclosinggreeting
        \par
        \vspace*{4pt}
        \closingbehandlet
        \feltclosingvedlegg
        \label{\currentSectionName} %set label for the last page before attachments
    \end{minipage}
}

% attachmentList
\newenvironment{attachmentList}
    {\par
    \addvspace{42pt minus 20pt} % TODO gjennomgang av design
    % enclose in brackets to prevent font from bleeding out to the rest of the letter.
    % Should be the same as the secondary letter title, but without extra spacing after.
    \titletwo\feltclosingvedleggprefix
    \standardtext
    \addvspace{6pt}
    \begin{itemize}}
    {\end{itemize}}


%----------------------------------------
%           signatures
%----------------------------------------
\NewTblrEnviron{signaturestable}
\SetTblrInner[signaturestable]{colspec={XX}}
\SetTblrInner[signaturestable]{colsep=0pt}
\SetTblrInner[signaturestable]{leftsep=0pt}
\SetTblrInner[signaturestable]{rightsep=0pt}
\SetTblrInner[signaturestable]{rowsep=0pt}
\SetTblrOuter[signaturestable]{postsep=0pt}
\SetTblrOuter[signaturestable]{footsep=0pt}

\newcommand{\doublesignature}[2]{
    \setlength{\tabcolsep}{12pt}
    \begin{signaturestable}{colspec={X[l]X[l]}, column{1} = {rightsep = 10mm},column{2} = {leftsep = 10mm}}
        #1 & #2 \\
        \feltclosingsaksbehandlersuffix & \feltclosingsaksbehandlersuffix \\
    \end{signaturestable}
}