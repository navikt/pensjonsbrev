...
class ServiceStatusTest {
    ...
    private fun serviceStatusApp(vararg services: ServiceStatus, block: suspend (client: HttpClient) -> Unit): Unit = createTestApplication { client ->
        application {
            routing {
    ...
    @Test
    fun `status responds`() = serviceStatusApp { client ->
    ...
    @Test
    fun `status for one available service`() = serviceStatusApp(AvailableService) { client ->
        val response = client.get("/status")
        assertEquals(HttpStatusCode.OK, response.status)
    ...
    @Test
    fun `status for one unavailable service`() = serviceStatusApp(UnavailableService) { client ->
        val response = client.get("/status")
        assertEquals(HttpStatusCode.OK, response.status)
    ...
    @Test
    fun `status for one available and one unavailable service`() = serviceStatusApp(AvailableService, UnavailableService) { client ->
        val response = client.get("/status")
        assertEquals(HttpStatusCode.OK, response.status)
    ...
    @Test
    fun `status for one failing and one available service`() = serviceStatusApp(AvailableService, FailingService) { client ->
        val response = client.get("/status")
        assertEquals(HttpStatusCode.OK, response.status)
        assertEquals(
