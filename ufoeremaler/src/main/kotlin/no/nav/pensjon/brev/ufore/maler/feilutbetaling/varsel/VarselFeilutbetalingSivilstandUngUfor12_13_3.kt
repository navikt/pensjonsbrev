        ...
        outline {
            paragraph {
                text(bokmal { + "Vi vurderer at du kan ha fått utbetalt for mye i uføretrygd fra og med " + fritekst("dato") +
                        " til og med " + fritekst("dato") + ". Grunnen til det, er at vi har fått opplysninger om at du har fått en samboer eller ektefelle. "})
            }
            paragraph {
                text(bokmal { + "Din sivilstand påvirker beregningen av din uføretrygd, og er årsaken til at vi vurderer at du har fått utbetalt for mye. " +
            ...
            paragraph {
                text(bokmal { + "Vi har den " + fritekst("dato") + " mottatt opplysninger fra " + fritekst("kilde ") + " om at du " + fritekst("handling") + " den " + fritekst("dato") + ". " })
            }
            paragraph {
                text(bokmal { + fritekst("dato") + " fikk du vedtak om innvilgelse av " + fritekst("uføregrad") + " prosent uføretrygd etter reglene for ung ufør. I søknaden dato fylte du ut at du var enslig.  " })
            }
            paragraph {
                text(bokmal { + "Du har uføretrygd med minsteytelse som ung ufør etter en sats på X som enslig. Sivilstanden din påvirker størrelsen på minsteytelsen din. " })
            }
            paragraph {
                text(bokmal { + "Du får en lavere utbetaling av uføretrygd fra måneden etter at sivilstanden din ble endret. Dette står i folketrygdloven § 22-12 femte ledd. " })
            }
            paragraph {
                text(bokmal { + "Du fikk utbetalt uføretrygd som enslig ung ufør i perioden " + fritekst("dato") + " til " + fritekst("dato") +
                        " , og her har det oppstått en feilutbetaling på " + pesysData.feilutbetaltBrutto.format(LocalizedFormatter.CurrencyFormat) +
