...
class TemplateModelSpecificationFactoryTest {
    ...
    private val spec = TemplateModelSpecificationFactory(AModel::class).build(emptyMap<String, Any>())
    ...
    fun `fails for Model classes without primary constructor`() {
        assertThrows<TemplateModelSpecificationError> { TemplateModelSpecificationFactory(NoPrimaryConstructor::class).build(emptyMap<String, Any>()) }
    }
    ...
    fun `fails for non data class`() {
        assertThrows<TemplateModelSpecificationError> { TemplateModelSpecificationFactory(NotADataClass::class).build(emptyMap<String, Any>()) }
    }
    ...
    fun `enum fields have Enum type with all enum-values`() {
        val spec = TemplateModelSpecificationFactory(WithEnumeration::class).build(emptyMap<String, Any>()).types[WithEnumeration::class.qualifiedName!!]!!
    ...
    fun `value class fields have Scalar type`() {
        val spec = TemplateModelSpecificationFactory(WithValueClass::class).build(emptyMap<String, Any>())
    ...
    fun `nullable fields are mapped correctly`() {
        val spec = TemplateModelSpecificationFactory(WithNullable::class).build(emptyMap<String, Any>())
    ...
    fun `recursive model structures causes failure`() {
        assertThrows<TemplateModelSpecificationError>("Should not support self-recursive types") {
            TemplateModelSpecificationFactory(Recursive::class).build(emptyMap<String, Any>())
        }
        assertThrows<TemplateModelSpecificationError>("Should not support circular-recursive types") {
            TemplateModelSpecificationFactory(SubRecursive::class).build(emptyMap<String, Any>())
        }
    }
