...
object LetterMarkupV2Module : SimpleModule() {
    @Suppress("unused")
    ...
    private fun textDeserializer() =
        object : StdDeserializer<LetterMarkupV2.Text>(LetterMarkupV2.Text::class.java) {
            override fun deserialize(p: JsonParser, ctxt: DeserializationContext): LetterMarkupV2.Text {
                val node = p.codec.readTree<JsonNode>(p)
                val type = when (val textType = LetterMarkupV2.Text.Type.valueOf(node.get("type").textValue())) {
    ...
    private fun settOppDeserialiseringFraInterfaceTilImplementasjon() {
        ...
        addAbstractTypeMapping<LetterMarkupWithDataUsageV2.Property, LetterMarkupWithDataUsageV2Impl.PropertyImpl>()
        addAbstractTypeMapping<LetterMarkupWithDataUsageV2, LetterMarkupWithDataUsageV2Impl>()
    }
}
