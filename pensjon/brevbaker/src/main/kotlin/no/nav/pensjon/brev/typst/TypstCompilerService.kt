package no.nav.pensjon.brev.typst
...
class TypstCompilerService(
    ...
    private fun settOppHttpClient(maxRetries: Int): HttpClient = HttpClient(CIO) {
        ...
        if (maxRetries > 0) {
            install(HttpRequestRetry) {
                ...
                delayMillis {
                    // Typst compiles in milliseconds, so use shorter delays than LaTeX
                    minOf(2.0.pow(it).toLong(), 200L) + Random.nextLong(50)
                }
    ...
    override suspend fun producePDF(pdfRequest: PDFRequest, path: String, shouldRetry: Boolean): PDFCompilationOutput =
        ...
        } ?: throw LatexTimeoutException("Spent more than $timeout trying to compile pdf with typst")
