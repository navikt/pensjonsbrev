...
import no.nav.pensjon.brev.api.model.maler.ufoerApi.endretUtPgaInntekt.selectors.endretUTPgaInntektDtoV2.barnetilleggFellesbarn.*
import no.nav.pensjon.brev.api.model.maler.ufoerApi.endretUtPgaInntekt.selectors.endretUTPgaInntektDtoV2.barnetilleggSaerkullsbarn.*
import no.nav.pensjon.brev.api.model.maler.ufoerApi.endretUtPgaInntekt.selectors.endretUTPgaInntektDtoV2.gjenlevendetillegg.*
...
object EndretUfoeretrygdPGAInntektFelles {
    ...
    data class Outline(val data: Expression<EndretUTPgaInntektDtoV2>) : OutlinePhrase<LangBokmalNynorsk>() {
        override fun OutlineOnlyScope<LangBokmalNynorsk, Unit>.template() {
            ...
            val barnetilleggFellesbarn = data.barnetilleggFellesbarn
            val barnetilleggSaerkullsbarn = data.barnetilleggSaerkullsbarn
            val gjenlevendetillegg = data.gjenlevendetillegg
            val totalNetto = data.totalNetto
            val forventetInntekt = data.forventetInntekt
            val virkningFom = data.virkningFom
            val datoForNormertPensjonsalder = data.datoForNormertPensjonsalder
            val btfbEndret = data.btfbEndret
            val btsbEndret = data.btsbEndret
            val brukerBorINorge = data.brukerBorINorge
            ...
            showIf(uforetrygd.endringsbelop.notEqualTo(0)) {
                ...
                paragraph {
                    text(
                        ...
                        nynorsk { + "Dersom du tener meir enn ditt botnfrådrag på " + uforetrygd.inntektsgrense.format(CurrencyFormat) +
                                " kroner per år, reduserer vi utbetalinga av uføretrygd. Det er berre den delen av inntekta di som er høgare enn botnfrådraget, som gir lågare utbetaling av uføretrygd. " }
                    )
                }
                paragraph {
                    text(
                ...
                ifNotNull(gjenlevendetillegg) {
                    ...
                    paragraph {
                        text(
                            ...
                            nynorsk { + "Du får attlevandetillegg i uføretrygda di. Endringa i di inntekt påverkar utbetalinga av attlevandetillegg. Attlevandetillegget endrast med same prosent som uføretrygda. " },
                        )
                    }
                }
