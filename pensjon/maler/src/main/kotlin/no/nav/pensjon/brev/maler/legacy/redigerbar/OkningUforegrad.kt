    ...
    override val template = createTemplate(
        ...
        title {
            text(
                ...
                nynorsk { +"Nav har innvilga søknaden din om auka uføretrygd" },
            )
        }
        outline {
            val pe = pesysData.pe
            ...
            paragraph {
                ...
                showIf(pe.vedtaksdata_kravhode_kravarsaktype().isNotAnyOf("omgj_etter_klage", "omgj_etter_anke")) {
                    ...
                    text(
                        bokmal { +"Du har fått medhold i klagen din, og får " + uforegradFraBeregning.format() + " prosent uføretrygd fra " + pe.vedtaksdata_virkningfom().format() + ". " },
                        nynorsk { +"Du har fått medhald i klaga di, og får " + uforegradFraBeregning.format() + " prosent uføretrygd frå " + pe.vedtaksdata_virkningfom().format() + ". " },
                    )
                }
                showIf(((pe.vedtaksdata_vilkarsvedtaklist_vilkarsvedtak_vilkar_unguforresultat()).equalTo("oppfylt"))) {
                    text(
                        bokmal { +"Hele uføretrygden din er innvilget med rettighet som ung ufør. " },
                        nynorsk { +"Heile uføretrygda di er innvilga med rett som ung ufør. " },
                    )
                }
            }
            ...
            showIf(((pe.vedtaksdata_vilkarsvedtaklist_vilkarsvedtak_vilkar_yrkesskaderesultat()).equalTo("oppfylt") and (pe.vedtaksdata_vilkarsvedtaklist_vilkarsvedtak_beregningsvilkar_ivsbegrunnelse()).equalTo("stdbegr_12_17_3_1"))) {
                paragraph {
                    text(
                        bokmal { +"Skadetidspunktet ditt har vi fastsatt til " + skadetidspunkt.format() + ". Din årlige inntekt på dette tidspunktet er fastsatt til " + pe.vedtaksdata_vilkarsvedtaklist_vilkarsvedtak_beregningsvilkar_ivsinntekt().format() + "." },
                        nynorsk { +"Vi har fastsett skadetidspunktet ditt til " + skadetidspunkt.format() + ". Den årlege inntekta di på dette tidspunktet er fastsett til " + pe.vedtaksdata_vilkarsvedtaklist_vilkarsvedtak_beregningsvilkar_ivsinntekt().format() + "." },
                    )
                }
            }
            ...
            showIf(((ifubegrunnelse).equalTo("stdbegr_12_8_2_1") and (ieuBegrunnelse).notEqualTo("stdbegr_12_8_1_3"))) {
                paragraph {
                    text(
                        bokmal { +"Inntekten din før du ble ufør er fastsatt til " + ifuinntekt.format() + " på uføretidspunktet " + uforetidspunkt.formatMonthYear() + ". " + fritekst("begrunnelse for fastsatt IFU") + ". " },
                        nynorsk { +"Inntekta di før du blei ufør, er fastsett til " + ifuinntekt.format() + " på uføretidspunktet " + uforetidspunkt.formatMonthYear() + ". " + fritekst("begrunnelse for fastsatt IFU") + ". " },
                    )
            ...
            showIf(((ifubegrunnelse).equalTo("stdbegr_12_8_2_5") and (ieuBegrunnelse).notEqualTo("stdbegr_12_8_1_3"))) {
                paragraph {
                    text(
                        bokmal { +"Du hadde begrenset yrkesaktivitet og inntekt før du ble ufør. Inntekten din før du ble ufør er fastsatt til " + ifuinntekt.format() + " på uføretidspunktet " + uforetidspunkt.formatMonthYear() + ". Dette er for å garantere deg et minstenivå på inntekt før uførhet. Dette minstenivået skal tilsvare 3,5 ganger folketrygdens grunnbeløp." },
            ...
            showIf((ieuBegrunnelse.equalTo("stdbegr_12_8_1_3"))) {
                paragraph {
                    text(
                        bokmal { +"Vi har fastsatt inntekten din før du ble ufør til " + ifuinntekt.format() + " på uføretidspunktet " + uforetidspunkt.formatMonthYear() + ". " + fritekst("begrunnelse for fastsatt IFU") + "." },
                        nynorsk { +"Vi har fastsett inntekta di før du blei ufør til " + ifuinntekt.format() + " på uføretidspunktet " + uforetidspunkt.formatMonthYear() + ". " + fritekst("begrunnelse for fastsatt IFU") + "." },
                    )
                    showIf(oifuMerEnnIfu) {
                        text(
                            bokmal { +" Oppjustert til virkningstidspunktet tilsvarer dette en inntekt på " + oifuKroner.format() + "." },
                ...
                paragraph {
                    text(
                        bokmal { +"Det er dokumentert at du har inntektsmuligheter som du ikke benytter. Disse tar vi med når vi fastsetter inntekten din etter at du ble ufør. Inntekten din etter at du ble ufør er fastsatt til " + ieuinntekt.format() + ", og uføregraden din er derfor fastsatt til " + uforegradFraBeregning.format() + " prosent." },
                        nynorsk { +"Det er dokumentert at du har inntektsmoglegheiter som du ikkje nyttar. Desse tek vi med når vi fastset inntekta di etter at du blei ufør. Inntekta di etter at du blei ufør, er fastsett til " + ieuinntekt.format() + ", og uføregraden din er derfor fastsett til " + uforegradFraBeregning.format() + " prosent." },
                    )
                }
            }
            ...
            showIf(((ifubegrunnelse).equalTo("stdbegr_12_8_2_2") or (ifubegrunnelse).equalTo("stdbegr_12_8_2_9"))) {
                paragraph {
                    text(
                        ...
                        nynorsk { +"Vi har fastsett inntekta di før du blei ufør til " + ifuinntekt.format() + ". Inntekta di før du blei ufør, er fastsett ut frå stillingsdelen din og forventa inntekt på " + ieuinntekt.format() + ". Inntekta di etter at du blei ufør, er derfor fastsett til " + ieuinntekt.format() + ", og uføregraden din blir " + uforegradFraBeregning.format() + " prosent." },
                    )
                }
            }
                    ...
                    paragraph {
                        ...
                        showIf(
                            (pe.ut_tbu1286_del1() and (btFellesNetto.equalTo(pe.vedtaksdata_beregningsdata_beregning_beregningytelsekomp_barnetilleggfelles_btfbbrutto()) and btSerkullNetto.equalTo(pe.vedtaksdata_beregningsdata_beregning_beregningytelsekomp_barnetilleggserkull_btsbbrutto())) and (btFellesNetto.notEqualTo(0) and btSerkullNetto.notEqualTo(0)) and ((pe.vedtaksdata_beregningsdata_beregningufore_beregningytelseskomp_barnetilleggserkull_btsbfradrag().equalTo(0) and pe.vedtaksdata_beregningsdata_beregningufore_beregningytelseskomp_barnetilleggfelles_btfbfradrag().equalTo(0)) or (pe.vedtaksdata_beregningsdata_beregningufore_beregningytelseskomp_barnetilleggserkull_btsbfradrag().greaterThan(0) and pe.vedtaksdata_beregningsdata_beregningufore_beregningytelseskomp_barnetilleggfelles_btfbfradrag().greaterThan(0))) and pe.vedtaksbrev_vedtaksdata_beregningsdata_beregningufore_beregningytelseskomp_barnetilleggserkull_avkortningsinformasjon_justeringsbelopperar()
                                .equalTo(0) and justeringsbelopperar0)
                        ) {
                            text(
                                bokmal { +" ikke" },
                                nynorsk { +" ikkje" },
