...
import no.nav.pensjon.brev.maler.legacy.pegruppe10.FUNKSJON_Month
import no.nav.pensjon.brev.maler.legacy.pegruppe10.FUNKSJON_Year
import no.nav.pensjon.brev.maler.legacy.pegruppe10.harOpptjeningUTMedOmsorg
import no.nav.pensjon.brev.maler.legacy.pegruppe10.harOpptjeningUTMedOpptjeningBruktAaretFoerOgFoerstegangstjeneste
import no.nav.pensjon.brev.maler.legacy.pegruppe10.pebrevkode
import no.nav.pensjon.brev.maler.legacy.pegruppe10.ut_forstegangstjenesteikkenull
import no.nav.pensjon.brev.maler.legacy.pegruppe10.ut_sisteopptjeningarlikuforetidspunkt
...
import no.nav.pensjon.brev.maler.legacy.pegruppe10.vedtaksdata_beregningsdata_beregningufore_beregningytelseskomp_uforetrygdordiner_ytelsesgrunnlag_beregningsgrunnlagordinar_opptjeningutliste_opptjeningut_forstegansgstjeneste
import no.nav.pensjon.brev.maler.legacy.pegruppe10.vedtaksdata_beregningsdata_beregningufore_beregningytelseskomp_uforetrygdordiner_ytelsesgrunnlag_beregningsgrunnlagordinar_opptjeningutliste_opptjeningut_omsorgsaar
...
data class TBU011V_TBU016V(val pe: Expression<PEgruppe10>): OutlinePhrase<LangBokmalNynorskEnglish>(){
            ...
            showIf(((FUNKSJON_Year(pe.vedtaksdata_vilkarsvedtaklist_vilkarsvedtak_beregningsvilkar_uforetidspunkt()).equalTo(FUNKSJON_Year(pe.vedtaksdata_vilkarsvedtaklist_vilkarsvedtak_beregningsvilkar_virkningstidpunkt())) and FUNKSJON_Month(pe.vedtaksdata_vilkarsvedtaklist_vilkarsvedtak_beregningsvilkar_uforetidspunkt()).lessThan(6) and FUNKSJON_Month(pe.vedtaksdata_vilkarsvedtaklist_vilkarsvedtak_beregningsvilkar_virkningstidpunkt()).lessThan(6)) and pe.vedtaksdata_beregningsdata_beregningufore_uforetrygdberegning_beregningsmetode().equalTo("folketrygd"))){
                ...
                paragraph {
                    text (
                        ...
                        english { + "The calculations only include income up to six times the National Insurance basic amount (G). The disability benefit equals 66 percent of the basis for calculation. You can find out how much the basic amount is at $GRUNNBELOEP_URL." },
                    )
                }
            }
            ...
            showIf(((FUNKSJON_Year(pe.vedtaksdata_vilkarsvedtaklist_vilkarsvedtak_beregningsvilkar_uforetidspunkt()).equalTo(FUNKSJON_Year(pe.vedtaksdata_vilkarsvedtaklist_vilkarsvedtak_beregningsvilkar_virkningstidpunkt())) and FUNKSJON_Month(pe.vedtaksdata_vilkarsvedtaklist_vilkarsvedtak_beregningsvilkar_uforetidspunkt()).lessThan(6) and FUNKSJON_Month(pe.vedtaksdata_vilkarsvedtaklist_vilkarsvedtak_beregningsvilkar_virkningstidpunkt()).lessThan(6)) and pe.vedtaksdata_beregningsdata_beregningufore_uforetrygdberegning_beregningsmetode().equalTo("folketrygd"))){
                ...
                paragraph {
                    text (
                        ...
                        english { + "Once the tax assessment is completed, your disability benefit will be recalculated. You will receive a new letter informing you of any new developments." },
                    )
                }
            }
                ...
                paragraph {
                    ...
                    showIf(pe.harOpptjeningUTMedOpptjeningBruktAaretFoerOgFoerstegangstjeneste()){
                        text (
                            ...
                            english { + "Your income was higher in the year prior to your initial service, and we have thus applied this incomed in the calculations." },
                        )
                    }
                }
            }
            ...
            showIf((pe.vedtaksdata_beregningsdata_beregningufore_uforetrygdberegning_beregningsmetode().equalTo("eos"))){
                ...
                paragraph {
                    text (
                        ...
                        english { + "When we calculate the average income, we only include the years you had income in Norway during this five-year period. If you had income in Norway and abroad during the same year, we apply the income that will benefit you most." },
                    )
                }
            }
        }
