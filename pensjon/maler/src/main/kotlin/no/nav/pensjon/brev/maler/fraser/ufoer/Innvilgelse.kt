        ...
        override fun OutlineOnlyScope<LangBokmalNynorsk, Unit>.template() {
            ...
            showIf(virkningbegrunnelseStdbegr_22_12_1_5) {
                ifNotNull(pe.vedtaksdata_kravhode_onsketvirkningsdato()) { virkningsdato ->
                    paragraph {
                        text(
                            bokmal { +"Du har fått innvilget uføretrygd fra " + virkningsdato.format() + ". Dette kaller vi virkningstidspunktet. Vi mottok søknaden din " + kravFremsattDato.ifNull(pe.vedtaksdata_kravhode_kravmottatdato()).format() + ". Dersom vilkårene for rett til uføretrygd var oppfylt før dette, kan uføretrygden innvilges opptil tre måneder før denne datoen. " },
                            nynorsk { +"Du har fått innvilga uføretrygd frå " + virkningsdato.format() + ". Dette kallar vi verknadstidspunktet. Vi fekk søknaden din " + kravFremsattDato.ifNull(pe.vedtaksdata_kravhode_kravmottatdato()).format() + ". Dersom vilkåra for rett til uføretrygd var oppfylte før dette, kan vi innvilge uføretrygd opptil tre månader før denne datoen. " },
                        )
                    }
                }
            }
