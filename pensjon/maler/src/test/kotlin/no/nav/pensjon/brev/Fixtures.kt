...
import no.nav.pensjon.brev.api.model.maler.legacy.OpplysningerOmBeregningenGPUtlandDtoSelectors
...
import no.nav.pensjon.brev.fixtures.redigerbar.createAvslagUfoerepensjonDto
...
import no.nav.pensjon.brev.fixtures.redigerbar.createInnvilgelseUfoeretrygdUtlandDto
...
import no.nav.pensjon.brev.fixtures.redigerbar.createVedtakEndringGjenlevendepensjonBosattUtlandDto
...
object Fixtures : LetterDataFactory {
    ...
    override fun <T : Any> create(letterDataType: KClass<T>): T =
        when (letterDataType) {
            ...
            VedtakEndringGjenlevendepensjonBosattUtlandDto::class -> createVedtakEndringGjenlevendepensjonBosattUtlandDto() as T
    ...
    override fun <T : Any> createVedlegg(letterDataType: KClass<T>): T = when(letterDataType) {
        ...
        PEgruppe10::class -> createPEgruppe10() as T
        OpplysningerOmBeregningenGPUtlandDto::class -> createOpplysningerOmBeregningGPUtlandDto() as T
        OversiktOverPensjonensStoerrelseGjenlevendepensjonDto::class -> createOversiktOverPensjonensStoerrelseGjenlevendepensjonDto() as T
        else -> throw IllegalArgumentException("Don't know how to construct: ${letterDataType.qualifiedName}")
    }
}