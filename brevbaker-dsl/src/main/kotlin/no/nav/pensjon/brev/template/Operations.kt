...
abstract class BinaryOperation<in In1, in In2, out Out>(val doc: Documentation? = null) : Operation() {
    ...
    class Equal<In> :
        BinaryOperation<In, In, Boolean>(Documentation("==", Documentation.Notation.INFIX)),
    ...
    class GreaterThan<in T : Comparable<T>> :
        BinaryOperation<T, T, Boolean>(Documentation(">", Documentation.Notation.INFIX)),
    ...
    class GreaterThanOrEqual<in T : Comparable<T>> :
        BinaryOperation<T, T, Boolean>(Documentation(">=", Documentation.Notation.INFIX)),
    ...
    class LessThanOrEqual<in T : Comparable<T>> :
        BinaryOperation<T, T, Boolean>(Documentation("<=", Documentation.Notation.INFIX)),
    ...
    class LessThan<in T : Comparable<T>> :
        BinaryOperation<T, T, Boolean>(Documentation("<", Documentation.Notation.INFIX)),
    ...
    object Or :
        BinaryOperation<Boolean, Boolean, Boolean>(Documentation("or", Documentation.Notation.INFIX)),
    ...
    object And :
        BinaryOperation<Boolean, Boolean, Boolean>(Documentation("and", Documentation.Notation.INFIX)),
    ...
    class IntPlus<T : IntValue>(val constructor: (Int) -> T) : BinaryOperation<T, T, T>(), StableHash by StableHash.of("BinaryOperation.IntPlus") {
        override fun apply(
            first: T,
            second: T,
        ): T = constructor(first.value + second.value)
    }

    class IfNull<T : Any> :
        BinaryOperation<T?, T, T>(Documentation("?:", Documentation.Notation.INFIX)),
    ...
    data class MapCollection<In1, In2, Out>(val mapper: BinaryOperation<In1, In2, Out>) : BinaryOperation<Collection<In1>, In2, Collection<Out>>() {
        override fun apply(
            first: Collection<In1>,
            second: In2,
        ): Collection<Out> = first.map { mapper.apply(it, second) }
    ...
    class EnumInList<EnumType : Enum<*>> : BinaryOperation<EnumType, List<EnumType>, Boolean>(), StableHash by StableHash.of("BinaryOperation.EnumInList") {
        override fun apply(
            first: EnumType,
            second: List<EnumType>,
        ): Boolean = second.contains(first)
    }
    ...
    class GetElementOrNull<ListType> : BinaryOperation<List<ListType>?, Int, ListType?>(), StableHash by StableHash.of("BinaryOperation.GetElementOrNull") {
        override fun apply(
            first: List<ListType>?,
            second: Int,
        ): ListType? = first?.getOrNull(second)
    }
    ...
    class IfElse<Out> : BinaryOperation<Boolean, Pair<Out, Out>, Out>(), StableHash by StableHash.of("BinaryOperation.IfElse") {
        override fun apply(
            first: Boolean,
            second: Pair<Out, Out>,
        ): Out = if (first) second.first else second.second
    }
    ...
    data class Flip<In1, In2, Out>(val operation: BinaryOperation<In2, In1, Out>) : BinaryOperation<In1, In2, Out>() {
        override fun apply(
            first: In1,
            second: In2,
        ): Out = operation.apply(second, first)
