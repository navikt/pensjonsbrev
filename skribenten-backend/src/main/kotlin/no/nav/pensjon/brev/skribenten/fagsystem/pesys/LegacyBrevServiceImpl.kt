...
class LegacyBrevServiceImpl(
    ...
    override suspend fun bestillOgRedigerEblankett(
        gjelderPid: Pid,
    ...
    private suspend fun redigerExstreamBrev(journalpostId: JournalpostId): Api.BestillOgRedigerBrevResponse =
        when (safService.waitForJournalpostStatusUnderArbeid(journalpostId)) {
            ...
            READY -> {
                penClient.redigerExstreamBrev(journalpostId)?.let { Api.BestillOgRedigerBrevResponse(url = it.uri) }
