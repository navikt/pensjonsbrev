    ...
    suspend fun hentSak(saksId: SaksId): Fagsak? =
        penClient.hentSak(saksId)?.let { sak ->
            Fagsak(
                ...
                sakType = sak.sakType,
                behandlingsnummer = behandlingsnummerMap[sak.sakType.kode] ?: null.also { logger.info("Spurte om sakstype som ikke har behandlingsnummer: ${sak.sakType} for sak ${sak.saksId}") },
                pid = sak.pid
            )
        }
