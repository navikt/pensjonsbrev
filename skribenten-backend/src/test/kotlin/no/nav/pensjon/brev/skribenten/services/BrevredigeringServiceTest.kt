...
class BrevredigeringServiceTest {
    ...
    private val templateDescription =
    ...
    private val brevdataResponseData =
    ...
    private val navAnsattService =
        mockk<NavansattService> {
            ...
            coEvery { hentNavansatt(eq(principalNavIdent.id)) } returns
                Navansatt(
                    emptyList(),
                    principalNavn,
                    "Hugo",
            ...
            coEvery { hentNavansatt(eq(principalNavIdent2.id)) } returns
                Navansatt(
                    emptyList(),
                    principalNavn2,
                    "Laurence",
    ...
    @Test
    fun `non existing brevredingering returns null`(): Unit =
        runBlocking {
            assertThat(brevredigeringService.hentBrev(saksId = sak.saksId, brevId = 99)).isNull()
        }

    @Test
    fun `can create and fetch brevredigering`(): Unit =
        runBlocking {
    ...
    @Test
    fun `can create brev for vedtak`(): Unit =
        runBlocking {
    ...
    @Test
    fun `brev must belong to provided saksId`(): Unit =
        runBlocking {
    ...
    @Test
    fun `cannot create brevredigering for a NavEnhet without access to it`(): Unit =
        runBlocking {
            val saksbehandlerValg = Api.GeneriskBrevdata().apply { put("valg1", true) }
            val result =
                withPrincipal(principalMock()) {
    ...
    @Test
    fun `can update brevredigering`(): Unit =
        runBlocking {
            val saksbehandlerValg = Api.GeneriskBrevdata().apply { put("valg1", true) }
    ...
    @Test
    fun `updates redigertBrev with fresh rendering from brevbaker`(): Unit =
        runBlocking {
            val saksbehandlerValg = Api.GeneriskBrevdata().apply { put("valg1", true) }
    ...
    @Test
    fun `cannot update non-existing brevredigering`(): Unit =
        runBlocking {
            val saksbehandlerValg = Api.GeneriskBrevdata().apply { put("valg1", true) }
            val oppdatert =
                withPrincipal(principalMock()) {
    ...
    @Test
    fun `does not wait for brevbaker before answering with brevredigering does not exist`(): Unit =
        runBlocking {
            val saksbehandlerValg =
                Api.GeneriskBrevdata().apply {
                    put("valg1", true)
    ...
    @Test
    fun `can delete brevredigering`(): Unit =
        runBlocking {
            val brev = opprettBrev().resultOrNull()!!
    ...
    @Test
    fun `delete brevredigering returns false for non-existing brev`(): Unit =
        runBlocking {
    ...
    @Test
    fun `hentPdf skal opprette et Document med referanse til Brevredigering`(): Unit =
        runBlocking {
            val brev = opprettBrev().resultOrNull()!!
    ...
    @Test
    fun `sletter relaterte documents ved sletting av brevredigering`(): Unit =
        runBlocking {
            val brev = opprettBrev().resultOrNull()!!
    ...
    @Test
    fun `dobbel oppretting av pdf er ikke mulig`(): Unit =
        runBlocking {
            val brev = opprettBrev().resultOrNull()!!
    ...
    @Test
    fun `Document redigertBrevHash er stabil`(): Unit =
        runBlocking {
            val brev = opprettBrev().resultOrNull()!!
    ...
    @Test
    fun `Document redigertBrevHash endres basert paa redigertBrev`(): Unit =
        runBlocking {
            val brev = opprettBrev().resultOrNull()!!
    ...
    @Test
    fun `hentPdf rendrer ny pdf om den ikke eksisterer`(): Unit =
        runBlocking {
            val brev = opprettBrev().resultOrNull()!!
    ...
    @Test
    fun `hentPdf rendrer ny pdf om den ikke er basert paa gjeldende redigertBrev`(): Unit =
        runBlocking {
            val brev = opprettBrev().resultOrNull()!!
    ...
    @Test
    fun `hentPdf rendrer ikke ny pdf om den er basert paa gjeldende redigertBrev`(): Unit =
        runBlocking {
            val brev = opprettBrev().resultOrNull()!!
            stagePdf("min første pdf".encodeToByteArray())
    ...
    @Test
    fun `attesterer hvis avsender har attestantrolle`(): Unit =
        runBlocking {
    ...
    @Test
    fun `attesterer ikke hvis avsender ikke har attestantrolle`(): Unit =
        runBlocking {
            Features.override(Features.attestant, true)
            clearMocks(brevbakerMock, penService)
    ...
    @Test
    fun `distribuerer sentralprint brev`(): Unit =
        runBlocking {
            clearMocks(brevbakerMock, penService)
    ...
    @Test
    fun `distribuerer ikke lokalprint brev`(): Unit =
        runBlocking {
            clearMocks(brevbakerMock, penService)
    ...
    @Test
    fun `brev kan reserveres for redigering gjennom opprett brev`(): Unit =
        runBlocking {
    ...
    @Test
    fun `brev kan reserveres for redigering gjennom hent brev`(): Unit =
        runBlocking {
    ...
    @Test
    fun `brev kan ikke oppdateres av andre enn den som har reservert det for redigering`(): Unit =
        runBlocking {
            val brev = opprettBrev(reserverForRedigering = true).resultOrNull()!!
    ...
    @Test
    fun `brev kan ikke endres om det er arkivert`(): Unit =
        runBlocking {
            val brev = opprettBrev(reserverForRedigering = true).resultOrNull()!!
            ...
            coEvery { penService.sendbrev(any(), any()) } returns
                ServiceResult.Ok(
    ...
    @Test
    fun `kan ikke sende brev som ikke er markert klar til sending`(): Unit =
        runBlocking {
            val brev = opprettBrev().resultOrNull()!!
    ...
    @Test
    fun `arkivert brev men ikke distribuert kan sendes`(): Unit =
        runBlocking {
            val brev = opprettBrev().resultOrNull()!!
    ...
    @Test
    fun `brev reservasjon utloeper`(): Unit =
        runBlocking {
            val brev = opprettBrev(reserverForRedigering = true).resultOrNull()!!
    ...
    @Test
    fun `brev reservasjon kan fornyes`(): Unit =
        runBlocking {
            val brev = opprettBrev(reserverForRedigering = true).resultOrNull()!!
    ...
    @Test
    fun `brev reservasjon kan frigis ved oppdatering`(): Unit =
        runBlocking {
            val brev = opprettBrev(reserverForRedigering = true).resultOrNull()!!
            assertThat(brev.info.redigeresAv).isEqualTo(principalNavIdent)
    ...
    @Test
    fun `brev reservasjon frigis ikke ved oppdatering`(): Unit =
        runBlocking {
            val brev = opprettBrev(reserverForRedigering = true).resultOrNull()!!
            assertThat(brev.info.redigeresAv).isEqualTo(principalNavIdent)
    ...
    @Test
    fun `oppdatering av redigertBrev endrer ogsaa redigertBrevHash`(): Unit =
        runBlocking {
            val brev = opprettBrev(reserverForRedigering = true).resultOrNull()!!
    ...
    @Test
    fun `kan overstyre mottaker av brev`(): Unit =
        runBlocking {
    ...
    @Test
    fun `kan fjerne overstyrt mottaker av brev`(): Unit =
        runBlocking {
            coEvery { samhandlerService.hentSamhandlerNavn(eq("samhandlerId")) } returns "hei"
            val mottaker = Dto.Mottaker.samhandler("samhandlerId")
    ...
    @Test
    fun `kan oppdatere mottaker av brev`(): Unit =
        runBlocking {
    ...
    @Test
    fun `kan sette annen mottaker for eksisterende brev`(): Unit =
        runBlocking {
            val brev = opprettBrev().resultOrNull()!!
    ...
    @Test
    fun `brev distribueres til annen mottaker`(): Unit =
        runBlocking {
    ...
    @Test
    fun `kan endre signerende saksbehandler signatur`(): Unit =
        runBlocking {
            val brev = opprettBrev().resultOrNull()!!
    ...
    @Test
    fun `kan tilbakestille brev`(): Unit =
        runBlocking {
            val brev =
                opprettBrev(
                    saksbehandlerValg =
            ...
            withPrincipal(principalMock()) {
                brevredigeringService.oppdaterBrev(
                    ...
                    nyttRedigertbrev =
                        brev.redigertBrev.copy(
                            blocks =
                                brev.redigertBrev.blocks +
                                    E_Paragraph(
                                        null,
                                        true,
            ...
            coEvery { brevbakerMock.getModelSpecification(eq(brev.info.brevkode)) } returns
                ServiceResult.Ok(
            ...
            assertThat(tilbakestilt.saksbehandlerValg).isEqualTo(
                Api.GeneriskBrevdata().apply {
                    put("ytelse", "uføre")
                    put("inkluderAfpTekst", false)
                    put("land", null)
    ...
    @Test
    fun `kan ikke markere brev klar til sending om ikke alle fritekst er fylt ut`(): Unit =
        runBlocking {
            coEvery {
                brevbakerMock.renderMarkup(
    ...
    @Test
    fun `kan markere brev klar til sending om alle fritekst er fylt ut`(): Unit =
        runBlocking {
            coEvery {
                brevbakerMock.renderMarkup(
                    eq(Testbrevkoder.TESTBREV), any(), any(), any(),
    ...
    private fun stagePdf(pdf: ByteArray) {
        coEvery { brevbakerMock.renderPdf(any(), any(), any(), any(), any()) } returns
            ServiceResult.Ok(
                LetterResponse(
                    file = pdf,
