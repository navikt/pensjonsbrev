...
@JvmName("formatSivilstandAvdodBestemtForm")
fun Expression<SivilstandAvdoed>.bestemtForm(storBokstav: Boolean = false) = format(formatter = SivilstandAvdodBestemt(storBokstav))
...
class SivilstandAvdodBestemt(private val storBokstav: Boolean) : LocalizedFormatter<SivilstandAvdoed>() {
    override fun apply(first: SivilstandAvdoed, second: Language): String = avdodSivilstand(first, second, true, storBokstav)
    override fun stableHashCode(): Int = "BorMedSivilstandBestemt($storBokstav)".hashCode()
}

class SivilstandAvdodUbestemt(private val storBokstav: Boolean) : LocalizedFormatter<SivilstandAvdoed>() {
    override fun apply(first: SivilstandAvdoed, second: Language): String = avdodSivilstand(first, second, false, storBokstav)
    override fun stableHashCode(): Int = "BorMedSivilstandUbestemt($storBokstav)".hashCode()
}
...
private fun avdodSivilstand(sivilstand: SivilstandAvdoed, language: Language, bestemtForm: Boolean, storBokstav: Boolean): String =
    when (sivilstand) {
        SivilstandAvdoed.GIFT -> when (language) {
            Bokmal, Nynorsk -> if (bestemtForm) "ektefellen" else "ektefelle"
            English -> "spouse"
        }

        SivilstandAvdoed.PARTNER -> when (language) {
            Bokmal -> if (bestemtForm) "partneren" else "partner"
            Nynorsk -> if (bestemtForm) "partnaren" else "partnar"
            English -> "partner"
        }

        SivilstandAvdoed.SAMBOER1_5, SivilstandAvdoed.SAMBOER3_2 -> when (language) {
            Bokmal -> if (bestemtForm) "samboeren" else "samboer"
            Nynorsk -> if (bestemtForm) "sambuaren" else "sambuar"
            English -> "cohabitant"
        }
