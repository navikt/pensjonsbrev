...
import no.nav.pensjon.brev.alder.model.vedlegg.MaanedligPensjonFoerSkattDtoSelectors.beregnetPensjonPerManedGjeldende
import no.nav.pensjon.brev.alder.model.vedlegg.MaanedligPensjonFoerSkattDtoSelectors.bruker
import no.nav.pensjon.brev.alder.model.vedlegg.MaanedligPensjonFoerSkattDtoSelectors.ektefelletilleggGjeldende
import no.nav.pensjon.brev.alder.model.vedlegg.MaanedligPensjonFoerSkattDtoSelectors.epsGjeldende
import no.nav.pensjon.brev.alder.model.vedlegg.MaanedligPensjonFoerSkattDtoSelectors.institusjonsoppholdGjeldende
import no.nav.pensjon.brev.alder.model.vedlegg.MaanedligPensjonFoerSkattDtoSelectors.krav
import no.nav.pensjon.brev.alder.model.vedlegg.MaanedligPensjonFoerSkattDtoSelectors.saerskiltSatsGjeldende
import no.nav.pensjon.brev.alder.model.vedlegg.MaanedligPensjonFoerSkattDtoSelectors.tilleggspensjonGjeldende
import no.nav.pensjon.brev.alder.model.vedlegg.MaanedligPensjonFoerSkattTabellSelectors.AlderspensjonPerManedSelectors.barnetilleggFBbrutto
import no.nav.pensjon.brev.alder.model.vedlegg.MaanedligPensjonFoerSkattTabellSelectors.AlderspensjonPerManedSelectors.barnetilleggSBbrutto
...
import no.nav.pensjon.brev.alder.model.vedlegg.MaanedligPensjonFoerSkattTabellSelectors.AlderspensjonPerManedSelectors.flyktningstatusErBrukt
import no.nav.pensjon.brev.alder.model.vedlegg.MaanedligPensjonFoerSkattTabellSelectors.AlderspensjonPerManedSelectors.fribelopET
import no.nav.pensjon.brev.alder.model.vedlegg.MaanedligPensjonFoerSkattTabellSelectors.AlderspensjonPerManedSelectors.fribelopFB
import no.nav.pensjon.brev.alder.model.vedlegg.MaanedligPensjonFoerSkattTabellSelectors.AlderspensjonPerManedSelectors.fribelopSB
...
val vedleggMaanedligPensjonFoerSkatt = createAttachment<LangBokmalNynorskEnglish, MaanedligPensjonFoerSkattDto>(
    ...
    outline = {
        ...
        showIf(regelverkstype.isOneOf(AP1967)) {
            ...
            paragraph {
                text(
                    bokmal { + "Reguleringen skjer med virkning fra 1. mai og selve økningen blir vanligvis etterbetalt i juni. Du får informasjon om dette på utbetalingsmeldingen din. På ${Constants.NAV_URL} kan du lese mer om hvordan pensjonene blir regulert." },
                    ...
                    english { + "The pension amount will be adjusted with effect from 1 May, and the actual increase is usually paid retroactively in June. You will be informed about this on your payment notice. You can read more about how pensions are adjusted at ${Constants.NAV_URL}" },
                )
            }
        }
