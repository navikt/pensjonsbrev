...
- `.github/skills/write-template/SKILL.md` — the full recipe (module selection, brevkode rules, Dto design, DSL body, fixture, registration, verification).
- If the brev is a `RedigerbarTemplate`, also read `.github/skills/write-template/write-redigerbar-template.md` for the `Saksbehandlervalg` / fagsystem-data Dto split, `kategori` / `brevkontekst` / `sakstyper`, and `fritekst(...)`.
