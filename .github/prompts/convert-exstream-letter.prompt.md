...
- `AGENTS.md`
- `skills/convert-exstream-letter.md` — the step-by-step pipeline for this task.
- `skills/write-template.md` — **read end-to-end, not just as a reference.** 
- `skills/write-redigerbar-template.md` — only if the letter contains `<FRITEKST: …>` markers (then it must be a `RedigerbarTemplate`).

**Inputs**
- Path to the converter-produced file: ${input:sourceFile}

**Hard rules**
- Follow the steps of `skills/convert-exstream-letter.md` in order. Do not skip ahead.
