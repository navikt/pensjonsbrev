  ...
  describe("at literal", () => {
    describe("next", () => {
      ...
      test("id of next block is kept if the specified is empty", () => {
        ...
        expect(select<AnyBlock>(result, mergeId).id).toEqual(
          select<AnyBlock>(state, { blockId: mergeId.blockId + 1 }).id,
        );
      });
    });
    ...
    describe("previous", () => {
      ...
      test("adjoining literal content in merging blocks are joined", () => {
        ...
        expect(select<TextContent>(result, { blockId: 0, contentId: 1 }).text).toEqual("lit1lit2");
      });
      ...
      describe("previous content in same block is itemList", () => {
        ...
        test("does not merge with previous block", () => {
          ...
          expect(result.editedLetter.letter.blocks[mergeId.blockId - 1]).toBe(
            withContentAfterList.editedLetter.letter.blocks[mergeId.blockId - 1],
          );
        });
        ...
        test("textcontent should be added to the last item", () => {
          ...
          expect(itemList.items[0].content[1]).toBe(
            select(withContentAfterList, { ...mergeId, contentId: mergeId.contentId + 1 }),
          );
        });
    ...
    describe("with next", () => {
      test("the specified items are merged and the number of items reduced", () => {
        ...
        const result = Actions.merge(
          state,
          { blockId: 0, contentId: 0, itemId: 0, itemContentId: 0 },
          MergeTarget.NEXT,
        );
      ...
      test("merge is ignored if the specified item is the last", () => {
        ...
        const result = Actions.merge(
          state,
          { blockId: 0, contentId: 0, itemId: 1, itemContentId: 0 },
          MergeTarget.NEXT,
        );
        expect(result).toBe(state);
      });
    ...
    describe("with previous", () => {
      ...
      describe("the current item is empty", () => {
        ...
        test("then the current is removed", () => {
          expect(select<ItemList>(result, { blockId: 0, contentId: 0 }).items).toHaveLength(1);
          expect(select<Item>(result, { blockId: 0, contentId: 0, itemId: 0 })).toBe(
            select<Item>(state, { blockId: 0, contentId: 0, itemId: 0 }),
          );
