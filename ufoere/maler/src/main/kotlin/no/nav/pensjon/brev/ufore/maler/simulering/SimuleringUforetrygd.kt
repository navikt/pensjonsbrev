...
import no.nav.pensjon.brev.ufore.api.model.maler.simulering.selectors.simuleringUforetrygdData.inntektsaar.*
import no.nav.pensjon.brev.ufore.api.model.maler.simulering.selectors.simuleringUforetrygdData.yrkesskade.*
import no.nav.pensjon.brev.ufore.maler.Brevkategori
import no.nav.pensjon.brev.ufore.maler.FeatureToggles
import no.nav.pensjon.brevbaker.api.model.LetterMetadata

@TemplateModelHelpers
object SimuleringUforetrygd : RedigerbarTemplate<SimuleringUforetrygdDto> {
    override val featureToggle = FeatureToggles.simulering.toggle
        ...
        outline {
            ...
            paragraph {
                table(
                    ...
                    row {
                        cell {
                            text(
                                bokmal { +"Årsbeløp fra " + pesysData.virkningstidspunkt.format() + " (før skatt):" },
                    ...
                    row {
                        ...
                        cell { text(bokmal { +pesysData.aarligBeloep.format() }) }
                    }
                    row {
                        cell {
                            text(
                                bokmal { +"Månedsbeløp fra " + pesysData.virkningstidspunkt.format() + " (før skatt):" },
                                fontType = FontType.BOLD,
                            )
                        }
                        cell {}
                    ...
                    row {
                        cell { text(bokmal { +"Ordinær uføretrygd" }) }
                        cell { text(bokmal { +pesysData.maanedligBeloep.format() }) }
                    }
                }
            }
            ...
            paragraph {
                table(
                    ...
                    row {
                        ...
                        cell { text(bokmal { +pesysData.grunnbeloep.format() }) }
                    }
                    row {
                        cell { text(bokmal { +"Trygdetid" }) }
                        cell { text(bokmal { +pesysData.trygdetidAar.format() + " år" }) }
                    }
                    row {
                        cell { text(bokmal { +"Virkningstidspunkt" }) }
                        cell { text(bokmal { +pesysData.virkningstidspunkt.format() }) }
                    }
                    row {
                    ...
                    row {
                        ...
                        cell { text(bokmal { +pesysData.uforegrad.format() + " %" }) }
                    }
                    row {
                        cell { text(bokmal { +"Snittet av de tre beste av de siste fem årene med pensjonsgivende inntekt" }) }
                        cell { text(bokmal { +pesysData.snittInntektTreBesteAvFem.format() }) }
                    }
                    ifNotNull(pesysData.yrkesskade) { ysk ->
                        row {
            ...
            paragraph {
                table(
                    ...
                    forEach(pesysData.inntektsgrunnlag) { rad ->
                        row {
