...
import no.nav.pensjon.brev.ufore.api.model.vedlegg.OpplysningerBruktIBeregningUTLegacyDto.Minsteytelse
...
import no.nav.pensjon.brev.ufore.api.model.vedlegg.selectors.opplysningerBruktIBeregningUTLegacyDto.minsteytelse.*
...
/**
 * Portert fra legacy TBU080V_TBU027V. "For deg som har rett til minsteytelse" inkl. satser,
 * ung ufoer, EOS-/bilateral-beregning. Hele frasen inkluderes naar [Visningsflagg.visMinsteytelse].
 * Dato-/brevkodestyrte satsblokker er erstattet med flagg.
 */
data class TBU080V_TBU027V(
    val flagg: Expression<Visningsflagg>,
    val minsteytelse: Expression<Minsteytelse>,
    ...
    override fun OutlineOnlyScope<LangBokmalNynorsk, Unit>.template() {
        ...
        // Block 1: fom < 2016-09-01 og virkningsdato >= 2024-07-01
        showIf(flagg.visMinsteytelseSatsBlokk1) {
            paragraph {
                text(
        ...
        // Block 2: fom < 2016-09-01 og virkningsdato < 2024-07-01
        showIf(flagg.visMinsteytelseSatsBlokk2) {
            paragraph {
                text(
        ...
        // Block 3: fom >= 2016-09-01 og virkningsdato >= 2024-07-01
        showIf(flagg.visMinsteytelseSatsBlokk3) {
            paragraph {
                text(
        ...
        // Block 4: fom >= 2016-09-01 og virkningsdato < 2024-07-01
        showIf(flagg.visMinsteytelseSatsBlokk4) {
            paragraph {
                text(
        ...
        // Ung ufoer >= 20 aar
        showIf(flagg.visUngUfoerOver20EtterJuli2024) {
            paragraph {
                text(
        ...
        }.orShowIf(flagg.visUngUfoerOver20FoerJuli2024) {
            paragraph {
                text(
        ...
        // Minsteytelse-sats (EOS vs ordinaer)
        showIf(flagg.visMinsteytelseEosSats) {
            ifNotNull(trygdetid.tellerTTEOS, trygdetid.nevnerTTEOS) { teller, nevner ->
                paragraph {
                    text(
                        bokmal { +"For deg vil minsteytelse utgjøre " + minsteytelse.sats.format(3) + " ganger folketrygdens grunnbeløp multiplisert med forholdstallet mellom norsk trygdetid og samlet EØS-trygdetid, " + teller.format() + "/" + nevner.format() + ". Er uføregraden din under 100 prosent, vil minsteytelsen bli justert ut fra uføregraden. Vi justerer også minsteytelsen ut fra trygdetid hvis du har mindre enn 40 års trygdetid. Dersom sivilstanden din endrer seg, kan det medføre at uføretrygden endres. " },
                        nynorsk { +"For deg vil minsteyting utgjere " + minsteytelse.sats.format(3) + " gonger grunnbeløpet i folketrygda multiplisert med forholdstallet mellom norsk trygdetid og samla EØS-trygdetid, " + teller.format() + "/" + nevner.format() + ". Er uføregraden din under 100 prosent, vil minsteytinga bli justert ut frå uføregraden. Vi justerer også minsteytinga ut frå trygdetida dersom du har mindre enn 40 års trygdetid. Dersom sivilstanden din endrar seg, kan det gjere at uføretrygda blir endra. " },
                    )
                }
            }
        }.orShow {
            paragraph {
                text(
                    ...
                    nynorsk { +"For deg vil minsteyting utgjere " + minsteytelse.sats.format(3) + " gonger grunnbeløpet i folketrygda. Er uføregraden din under 100 prosent, vil misteytinga bli justert ut frå uføregraden. Vi justerer også minsteytinga ut frå trygdetida, dersom du har mindre enn 40 års trygdetid. Du må melde frå til Nav om sivilstanden din endrar seg, dette kan gjere at uføretrygda blir endra." },
                )
            }
        }

        // Ung ufoer under 20 - fremtidig beregning fra fylte 20
        showIf(flagg.visUngUfoerUnder20Fremtid) {
            showIf(flagg.visUngUfoerUnder20FremtidEtterJuli2024) {
                paragraph {
                    text(
                    ...
                    showIf(flagg.borMedEktefelleEllerSamboer) {
                        text(bokmal { +"2,709" }, nynorsk { +"2,709" })
                    }.orShow {
                        text(bokmal { +"2,959" }, nynorsk { +"2,959" })
                    }
                    text(
                ...
                paragraph {
                    ...
                    showIf(flagg.borMedEktefelleEllerSamboer) {
                        text(bokmal { +"2,66" }, nynorsk { +"2,66" })
                    }.orShow {
                        text(bokmal { +"2,91" }, nynorsk { +"2,91" })
                    }
                    text(
        ...
        // Halv minsteytelse (faktisk trygdetid i Norge)
        showIf(flagg.visHalvMinsteytelse) {
            paragraph {
                text(
        ...
        // EOS - ikke sammenlegging
        showIf(flagg.visEosBeregningIkkeSammenlegging) {
            paragraph {
                text(
        ...
        // EOS/nordisk - sammenlegging
        showIf(flagg.visEosBeregningSammenlegging) {
            paragraph {
                text(
        ...
        // Bilateral trygdeavtale - sammenlegging
        showIf(flagg.visBilateralBeregningSammenlegging) {
            ifNotNull(trygdetid.avtaleland) { avtaleland ->
                paragraph {
                    text(
                        bokmal { +"Vi beregner uføretrygden din etter bestemmelsene i trygdeavtalen med " + avtaleland + " fordi vilkårene for rett til uføretrygd er oppfylt ved sammenlegging av opptjeningstid i Norge og " + avtaleland + ". Uføretrygden din er først beregnet etter samlet opptjening i Norge og " + avtaleland + ". Deretter er uføretrygden multiplisert med et forholdstall, som angir forholdet mellom faktisk opptjeningstid i Norge og samlet faktisk opptjening i Norge og " + avtaleland + ". Du kan se hvilke opplysninger vi har brukt i beregningen i tabellen " + quoted("Opplysninger vi har brukt i beregningen") + "." },
                        nynorsk { +"Vi bereknar uføretrygda di etter reglane i trygdeavtalen med " + avtaleland + " fordi vilkåra for rett til uføretrygd er oppfylt når ein legg saman oppteningstid i Noreg og " + avtaleland + ". Uføretrygda di er først berekna etter samla opptening i Noreg og " + avtaleland + ". Deretter er uføretrygda multiplisert med eit forholdstal som angir forholdet mellom faktisk oppteningstid i Noreg og samla faktisk opptening i Noreg og " + avtaleland + ". Du kan sjå kva opplysningar vi har brukt i berekninga i tabellen " + quoted("Opplysningar vi har brukt i berekninga") + "." },
                    )
                }
            }
        }

        // Folketrygd - trygdeavtale vurdert
        showIf(flagg.visFolketrygdVurdertTrygdeavtale) {
            paragraph {
                text(
