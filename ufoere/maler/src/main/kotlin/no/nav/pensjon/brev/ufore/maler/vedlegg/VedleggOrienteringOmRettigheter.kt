...
object VedleggInnsynSakUfoeretrygdPesys : OutlinePhrase<LangBokmalNynorskEnglish>() {
    override fun OutlineOnlyScope<LangBokmalNynorskEnglish, Unit>.template() {
        ...
        paragraph {
            text(
                bokmal { + "Med få unntak har du rett til å se dokumentene i saken din. Du kan logge deg inn via ${Constants.NAV_URL} for å se dokumenter i saken din. Du kan også ringe oss på telefon ${Constants.NAV_KONTAKTSENTER_TELEFON_UFORE}." },
                nynorsk { + "Med få unntak har du rett til å sjå dokumenta i saka di. Du kan logge deg inn via ${Constants.NAV_URL} for å sjå dokumenter i saka di. Du kan også ringje oss på telefon ${Constants.NAV_KONTAKTSENTER_TELEFON_UFORE}." },
                english { + "With some exceptions, you are entitled to access the documents relating to your case. Log on to ${Constants.NAV_URL} to review the documents in connection with your case. You can also call us at telephone +47 ${Constants.NAV_KONTAKTSENTER_TELEFON_UFORE}." },
            )
        }
    }
}
...
data class VedleggKlagePaaVedtaket(val telefonnummer: Expression<Telefonnummer>) : OutlinePhrase<LangBokmalNynorskEnglish>() {
    override fun OutlineOnlyScope<LangBokmalNynorskEnglish, Unit>.template() {
        ...
        paragraph {
            text(
                bokmal { + "Klagen må inneholde navn, fødselsnummer og adresse. Bruk gjerne skjemaet som du finner på ${Constants.KLAGE_URL}. Trenger du hjelp, er du velkommen til å ringe oss på telefon " + telefonnummer.format() + "." },
                nynorsk { + "Klaga må innehalde namn, fødselsnummer og adresse. Bruk gjerne skjemaet som du finn på ${Constants.KLAGE_URL}. Treng du hjelp, er du velkomen til å ringje oss på telefon " + telefonnummer.format() + "." },
                english { + "Your appeal must include your name, national identity number and address. Feel free to use the form found at ${Constants.KLAGE_URL}. Should you need assistance in writing the appeal, please call us at tel.: " + telefonnummer.format() + "." }
            )
        }
        paragraph {
