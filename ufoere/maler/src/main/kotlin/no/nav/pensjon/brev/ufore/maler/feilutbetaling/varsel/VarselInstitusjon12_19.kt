...
object VarselInstitusjon12_19: RedigerbarTemplate<FeilutbetalingSpesifikkVarselDto> {
    ...
    override val template = createTemplate(
        ...
        val institusjon = fritekst("institusjon")
        ...
        outline {
            paragraph {
                text(bokmal { + "Vi vurderer at du kan ha fått utbetalt for mye i uføretrygd fra og med " + dato +
                        " til og med " + dato + " . Grunnen til det, er at vi har fått opplysninger " +
                        "om at du har hatt fått dekket kost og losji over en lengre periode.  " },
                    nynorsk { + "Vi vurderer at du kan ha fått utbetalt for mye i uføretrygd fra og med " + dato +
                            " til og med " + dato + " . Grunnen til det, er at vi har fått opplysninger " +
                            "om at du har hatt fått dekket kost og losji over en lengre periode.  "})
            }
            paragraph {
                text(
                    ...
                    nynorsk { + "Uføretrygden påvirkes av om du får dekket kost og losji lengre enn 3 måneder. Dette gjelder også for opphold på institusjon i utlandet. "}
                )
            }
            paragraph {
                text(
                    bokmal { + "I vedtaket av " + dato + ", informerte vi deg om at opphold på institusjon med kost og losji, " +
                            "kan ha ført til at du har fått utbetalt for mye uføretrygd tilbake i tid.  " },
                    nynorsk { + "I vedtaket av " + dato + ", informerte vi deg om at opphold på institusjon med kost og losji, " +
                            "kan ha ført til at du har fått utbetalt for mye uføretrygd tilbake i tid.  "}
                )
            }
            paragraph {
                text(
                    ...
                    nynorsk { + "Utbetalingen av uføretrygd skal ikke bli lavere hvis du forsørger ektefelle eller barn. " +
                            "Hvis du har hatt faste og nødvendige utgifter til bolig (husleie, strøm og/eller innboforsikring) i perioden vi mener du " +
                            "fikk utbetalt for mye, kan vi gjøre en ny vurdering av feilutbetalingen. Du må sende oss dokumentasjon på dine utgifter. "}
                )
            }
            ...
            title1 {
                text(
                    ...
                    nynorsk { + "Derfor mener vi du har fått utbetalt for mye "}
                )
            }
            paragraph {
                text(
                    bokmal { + "Vi har den " + dato + " fått opplysninger fra " + kilde + " om at du var innlagt ved " +
                            institusjon + " fra " + dato + ". " + fritekst("Evt. Til dato/idag") },
                    nynorsk { + "Vi har den " + dato + " fått opplysninger fra " + kilde + " om at du var innlagt ved " +
                            institusjon + " fra dato. " + fritekst("Evt. Til dato/idag") }
                )
            }
            paragraph {
                text(
                    bokmal { + "Du fikk utbetalt uføretrygd i perioden " + dato + " til " + dato +
                            ...
                            "vært lavere i denne perioden, og derfor har det skjedd en feilutbetaling på " + bruttoFeilutbetalt + " kroner.  " },
                nynorsk { + "Du fikk utbetalt uføretrygd i perioden " + dato + " til " + dato +
                        " samtidig som du hadde et lengre opphold på institusjon hvor du fikk dekket kost og losji. Utbetalingen din skulle" +
                        " vært lavere i denne perioden, og derfor har det skjedd en feilutbetaling på " + bruttoFeilutbetalt + " kroner.  "}
                )
            }
